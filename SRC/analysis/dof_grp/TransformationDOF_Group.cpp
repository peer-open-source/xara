...
int
TransformationDOF_Group::getNumDOF() const
{
  return modNumDOF;
}
...
int
TransformationDOF_Group::getNumFreeDOF() const
{
  if (modID != 0) {
...
int
TransformationDOF_Group::getNumConstrainedDOF() const
{
  if (modID != 0) {
    int numConstr = 0;
    for (int i=0; i<modNumDOF; i++)
...
const Vector &
TransformationDOF_Group::getCommittedAccel()
{
...
const Vector &
TransformationDOF_Group::getTrialDisp()
{
...
const Vector &
TransformationDOF_Group::getTrialVel()
{
...
const Vector &
TransformationDOF_Group::getTrialAccel()
{
...
{
  const int dof = theSP.getDOF_Number();
  theSPs[dof] = &theSP;
      ...
      if (theMP != nullptr) {
        ...
        if (T != nullptr) {
          ...
          for (int i = 0; i < constrainedDOF.Size(); i++) {
            const int cDOF = constrainedDOF(i);
