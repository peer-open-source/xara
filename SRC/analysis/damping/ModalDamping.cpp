...
#if 0
int
ModalDamping::addModalDampingMatrix(TransientIntegrator& integrator) 
{
  int res = 0;
  ...
  double cFactor = integrator.getCFactor();
  if (cFactor == 0)
    return 0;
  ...
  for (int dof = 0; dof<numDOF; dof++) {
    ...
    for (int i=0; i<numModes; i++) {
      ...
      if (V[i] > 0) {
        ...
        if (ei_dof != 0.0) {
          ...
          double beta = V[i] * ei_dof * cFactor;
    ...
    if (zeroCol == false)
      if (theSOE->addColA(*dampingForces, dof, 1.0) != 0) {
        // if the SOE doesnt implement addColA, just break
        break;
      }
