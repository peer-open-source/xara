...
{
  return new NormDispOrUnbalance(this->tolDisp,
                                    this->tolUnbalance,
                                    iterations,
                                    this->printFlag,
                                    this->nType,
...
NormDispOrUnbalance::test(LinearSOE& theSOE)
{
  ...
  // algo failed to converged after specified number of iterations - but RETURN OK
    ...
    if (printFlag & ConvergenceTest::PrintFailure) {
        pstream << "WARNING Failed to converge with criteria NormDispOrUnbalance but going on - ";
        pstream << ", NormX: " << normX;
        pstream << ", NormB: " << normB  << ", NormIncr: " << numIncr << "\n";
    }
    return currentIter;
  ...
  // algo failed to converged after specified number of iterations - return FAILURE -2
    ...
    pstream << "WARNING Failed to converge with criteria NormDispAndUnbalance \n";
    pstream << "after: " << pad(currentIter) << " iterations\n";
    currentIter++;
...
int
NormDispOrUnbalance::getMaxNumTests()
{
  return maxNumIter;
}


double NormDispOrUnbalance::getRatioNumToMax()
{
