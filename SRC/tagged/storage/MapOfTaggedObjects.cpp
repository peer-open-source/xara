...
{
    MAP_TAGGED::iterator  theEle;
    int tag = newComponent->getTag();

    // check if the ele already in map, if not we add
    std::pair<MAP_TAGGED::iterator ,bool> res = theMap.insert(MAP_TAGGED::value_type (tag,newComponent));    
    if (res.second == false) {
      opserr << "MapOfTaggedObjects::addComponent - "
    ...
    if (theEle == theMap.end()) {
      theMap.insert(MAP_TAGGED::value_type (tag,newComponent));
...
int
MapOfTaggedObjects::getNumComponents() const
{
...
TaggedObjectStorage *
MapOfTaggedObjects::getEmptyCopy()
{
  return new MapOfTaggedObjects();
}
...
MapOfTaggedObjects::clearAll(bool invokeDestructor)
{
  ...
  if (invokeDestructor == true) {
    MAP_TAGGED::iterator  p = theMap.begin();
    while (p != theMap.end()) {
      delete (*p).second;
      p++;
    }
