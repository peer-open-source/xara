...
:BJMaterial(tag, ND_TAG_FiniteDeformationEP3D)
{
	if (fde3d_in)
	  fde3d = fde3d_in->getCopyBJ();
	else {
...
//----------------------------------------------------------------------
BJMaterial* FiniteDeformationEP3D::getCopyBJ (void)
{
	BJMaterial* tmp = new FiniteDeformationEP3D(
	  this->getTag(),
	  this->getFDE3D(),
	  this->getFDY(),
	  this->getFDF(),
...
//----------------------------------------------------------------------
BJMaterial* FiniteDeformationEP3D::getCopyBJ (const char *code)
{
	...
	{
	  opserr << "FiniteDeformationEP3D::getCopyBJ fainled:" << code << "\n";
	  exit(-1); 
	}                                                   
}
...
//----------------------------------------------------------------------
BJMaterial * FiniteDeformationEP3D::getFDE3D() const
{
	return fde3d;
}
...
int FiniteDeformationEP3D::ImplicitAlgorithm()
{
    ...
    if ( yieldfun > (fdy->getTolerance()) ) { // start of plastic part
      ...
      } while (yieldfun > fdy->getTolerance() || 
                residual > tolerance && iter_counter < Max_Iter ); // end of do - while
