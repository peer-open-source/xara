...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_ConcreteL01Material)
{
...
ConcreteL01::ConcreteL01(int tag, double FPC, double EPSC0)
...
ConcreteL01::ConcreteL01() : UniaxialMaterial(0, MAT_TAG_ConcreteL01), fpc(0.0), epsc0(0.0)
{
  // Set trial values
  this->revertToStart();
}
...
int
ConcreteL01::setTrialStrain(double strain, double strainRate)
{
  /* FMK
   X = x;
   K = k;
...
int
ConcreteL01::setTrial(double strain, double& stress, double& tangent, double strainRate)
{
  /* FMK
   X = x;
   K = k;
...
double
ConcreteL01::getStrain()
{
  return Tstrain;
}

double
ConcreteL01::getStress()
{
  return Tstress;
}

double
ConcreteL01::getTangent()
{
  return Ttangent;
}

double
ConcreteL01::getSecant()
{
  double TSecant;
  if (Tstrain == 0.0) {
...
int
ConcreteL01::revertToLastCommit()
{
...
void
ConcreteL01::determineTrialState(double dStrain)
{
  double Ec0 = 1.4 * fpc / epsc0;
  if (TloadingState == 0) {
    envelope();
  ...
  if (TloadingState == 1) {
    ...
    opserr << " ConcreteL01::determineTrialState -- impropter TloadingState: " << TloadingState
           << "\n";
...
ConcreteL01::pathFive()
{
  ...
  if (reloadPath == 1) {
    ...
    opserr << " ConcreteL01::pathFive -- improper reloadPath : " << reloadPath << "\n";
  }
}


void
ConcreteL01::pathSeven()
{
...
void
ConcreteL01::getApproachFiveToComStrain()
{
  approachFiveToComStrain = 0.0;
  ...
  if (tempB * tempB - 4.0 * tempC < 0.0) {
    opserr << " ConcreteL01::getApproachFiveToComStrain -- can not get root of equation: sqrt(x) "
              "x<0! \n";
  }
  ...
  } else // intersection occurs at descending branch
  {
    ...
    if ((tempK * zeta * epsc0 + b) < zeta * D * fpc) {
      opserr << " ConcreteL01::getApproachFiveToComStrain -- No intersection of reloading path "
...
void
ConcreteL01::getApproachSixToComStrain()
{
  approachSixToComStrain = 0.0;
...
int
ConcreteL01::sendSelf(int commitTag, Channel& theChannel)
{
  int res = 0;
...
int
ConcreteL01::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;
  static Vector data(21);
...
Response*
ConcreteL01::setResponse(const char** argv, int argc, OPS_Stream& theOutput)
{
...
void
ConcreteL01::Print(OPS_Stream& s, int flag)
{
