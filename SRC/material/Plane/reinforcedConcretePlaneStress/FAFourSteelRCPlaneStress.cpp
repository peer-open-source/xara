...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_FAFourSteelRCPlaneStressMaterial)
{
  if (numFAFourSteelRCPPlaneStressMaterials == 0) {
    numFAFourSteelRCPPlaneStressMaterials++;
    opslog << "FAFourSteelRCPPlaneStress unaxial material - Written by J.Zhong, Thomas T.C. Hsu "
              "and Y.L. Mo - Copyright@2009\n";
  }
  ...
  if (numRemainingArgs < 20) {
    opserr << "Want: nDMaterial RAFourSteelRCPlaneStress matTag? rho? UniaxiaMatTag1? "
              "UniaxiaMatTag2? UniaxiaMatTag3? UniaxiaMatTag4? UniaxiaMatTag5? UniaxiaMatTag6? "
...
FAFourSteelRCPlaneStress ::FAFourSteelRCPlaneStress(int tag,
                                                    ...
                                                    double E,
                                                    double EPSC0)
 : NDMaterial(tag, ND_TAG_FAFourSteelRCPlaneStress),
   rho(RHO),
   angle1(ANGLE1),
   angle2(ANGLE2),
   angle3(ANGLE3),
   ...
   tangent_matrix(3, 3)
{
  ...
  if (theMaterial == 0) {
    opserr << " FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress - failed to allocate material "
              "array\n";
    exit(-1);
  }
  ...
  // Check allocation
  if (theMaterial[0] == 0) {
    opserr << " FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress - failed to get a copy for "
              "steel1\n";
    exit(-1);
  }
  ...
  if ((theResponses[0] == 0) || (theResponses[1] == 0) || (theResponses[2] == 0) ||
    ...
    opserr << " FAFourSteelRCPLaneStress::FAFourSteelRCPlaneStress - failed to set appropriate "
              "materials tag:"
           << tag << endln;
    exit(-1);
  }
...
FAFourSteelRCPlaneStress::FAFourSteelRCPlaneStress()
 : NDMaterial(0, ND_TAG_FAFourSteelRCPlaneStress),
   strain_vec(3),
   stress_vec(3),
   tangent_matrix(3, 3)
{
...
double
FAFourSteelRCPlaneStress::getRho(void)
{
  return rho;
}
...
int
FAFourSteelRCPlaneStress::setTrialStrain(const Vector& v)
{
...
///*
int
FAFourSteelRCPlaneStress::setTrialStrain(const Vector& v, const Vector& r)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
int
FAFourSteelRCPlaneStress::setTrialStrainIncr(const Vector& v)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
int
FAFourSteelRCPlaneStress::setTrialStrainIncr(const Vector& v, const Vector& r)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
const Matrix&
FAFourSteelRCPlaneStress::getTangent(void)
{
  return tangent_matrix;
}

const Vector&
FAFourSteelRCPlaneStress::getStress(void)
{
...
const Vector&
FAFourSteelRCPlaneStress ::getStrain()
{
  return strain_vec;
}
...
const Vector&
FAFourSteelRCPlaneStress::getCommittedStress(void)
{
  return stress_vec;
}
...
const Vector&
FAFourSteelRCPlaneStress::getCommittedStrain(void)
{
  return strain_vec;
}
...
int
FAFourSteelRCPlaneStress::commitState(void)
{
  for (int i = 0; i < 6; i++) {
...
int
FAFourSteelRCPlaneStress::revertToLastCommit(void)
{
...
int
FAFourSteelRCPlaneStress::revertToStart(void)
{
  int i;
  for (i = 0; i < 6; i++) {
...
NDMaterial*
FAFourSteelRCPlaneStress::getCopy(void)
{
  /*
  ...
  FAFourSteelRCPlaneStress* theCopy = new FAFourSteelRCPlaneStress(
      this->getTag(), rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3],
      theMaterial[4], theMaterial[5], angle1, angle2, angle3, angle4, rou1, rou2, rou3, rou4, fpc,
      fy, E0, epsc0);
...
NDMaterial*
FAFourSteelRCPlaneStress::getCopy(const char* type)
{
  /*
	FAFourSteelRCPlaneStress *clone;
	clone = new FAFourSteelRCPlaneStress();
	*clone = *this;
	return clone;
	//*/
  FAFourSteelRCPlaneStress* theModel = new FAFourSteelRCPlaneStress(
      this->getTag(), rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3],
      theMaterial[4], theMaterial[5], angle1, angle2, angle3, angle4, rou1, rou2, rou3, rou4, fpc,
      fy, E0, epsc0);
...
void
FAFourSteelRCPlaneStress::Print(OPS_Stream& s, int flag)
{
...
int
FAFourSteelRCPlaneStress::sendSelf(int commitTag, Channel& theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FAFourSteelRCPlaneStress::sendSelf() - " << this->getTag()
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (res < 0) {
      opserr << "FAFourSteelRCPlaneStress::sendSelf() - " << this->getTag()
...
int
FAFourSteelRCPlaneStress::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FAFourSteelRCPlaneStress::recvSelf() - " << this->getTag()
  ...
  if (theMaterial == 0) {
    ...
    if (theMaterial == 0) {
      opserr
          << "FAFourSteelRCPlaneStress::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 6; i++) {
      ...
      if (theMaterial[i] == 0) {
        opserr << "FAFourSteelRCPlaneStress::recvSelf() - Broker could not create NDMaterial of "
                  "class type "
               << matClassTag << endln;
        return -1;
      }
      ...
      if (res < 0) {
        opserr << "FAFourSteelRCPlaneStress::recvSelf() - material " << i
...
int
FAFourSteelRCPlaneStress::determineTrialStress(void)
{
  ...
  } else // Tstrain[0] != Tstrain[1]
  {
    ...
    if (fabs(Tstrain[2]) < 1e-7) {
      ...
      opserr
          << "FAReinforceConcretePlaneStress::determineTrialStress: Failure to calculate citaR\n";
...
double
FAFourSteelRCPlaneStress::getAngleError(double inputCita)
{
  double pi = 3.14159265359;
  double outputCita;
...
double
FAFourSteelRCPlaneStress::getPrincipalStressAngle(double inputAngle)
{
  ...
  double stressS1, stressS2, stressS3,
      stressS4; // stress of steel layers in 1, 2, 3 and 4 direction
  ...
  strainS1 = pow(cos(citaS1), 2) * Tstrain[0] + pow(sin(citaS1), 2) * Tstrain[1] +
             2.0 * sin(citaS1) * cos(citaS1) * Tstrain[2];
  strainS2 = pow(cos(citaS2), 2) * Tstrain[0] + pow(sin(citaS2), 2) * Tstrain[1] +
             2.0 * sin(citaS2) * cos(citaS2) * Tstrain[2];
  strainS3 = pow(cos(citaS3), 2) * Tstrain[0] + pow(sin(citaS3), 2) * Tstrain[1] +
             2.0 * sin(citaS3) * cos(citaS3) * Tstrain[2];
  strainS4 = pow(cos(citaS4), 2) * Tstrain[0] + pow(sin(citaS4), 2) * Tstrain[1] +
             2.0 * sin(citaS4) * cos(citaS4) * Tstrain[2];


  if (strainSF < strainS1) {
    strainSF = strainS1;
  }
  if (strainSF < strainS2) {
    strainSF = strainS2;
  }
  if (strainSF < strainS3) {
    strainSF = strainS3;
  }
  if (strainSF < strainS4) {
    strainSF = strainS4;
  }
  ...
  if ((epslonOne > 0.0) && (epslonTwo > 0.0)) {
    ...
    opserr << "FAReinforceConcretePlaneStress::getPrincipalStressAngle: failure to get Hsu/Zhu "
              "ratio!\n";
  }
  ...
  if (v12 * v21 == 1.0) {
    opserr
        << "FAReinforceConcretePlaneStress::getPrincipalStressAngle: failure to get matrix [V]!\n";
  ...
  Tstress[2] =
      cos(citaIn) * sin(citaIn) * cigmaOneC - cos(citaIn) * sin(citaIn) * cigmaTwoC +
  ...
  } else // Tstrain[0] != Tstrain[1]
  {
    ...
    if (fabs(Tstress[2]) < 1e-7) {
      ...
      opserr << "FAReinforceConcretePlaneStress::getPrincipalStressAngle: Failure to calculate "
