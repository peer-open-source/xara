...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_RAFourSteelRCPlaneStressMaterial)
{
  if (numRAFourSteelRCPPlaneStressMaterials == 0) {
    numRAFourSteelRCPPlaneStressMaterials++;
    opslog << "RAFourSteelRCPPlaneStress unaxial material - Written by J.Zhong, Thomas T.C. Hsu "
...
RAFourSteelRCPlaneStress::RAFourSteelRCPlaneStress(int tag,
                                                   ...
                                                   double E,
                                                   double EPSC0)
 : NDMaterial(tag, ND_TAG_RAFourSteelRCPlaneStress),
   rho(RHO),
   angle1(ANGLE1),
   angle2(ANGLE2),
   angle3(ANGLE3),
   ...
   tangent_matrix(3, 3)
{
  ...
  if (theMaterial == 0) {
    opserr << " RAFourSteelRCPlaneStress::RAFourSteelRCPlaneStress - failed to allocate material "
              "array\n";
    exit(-1);
  }
  ...
  // Check allocation
  if (theMaterial[0] == 0) {
    opserr << " RAFourSteelRCPlaneStress::RAFourSteelRCPlaneStress - failed to get a copy for "
              "steel1\n";
    exit(-1);
  }
  ...
  if ((theResponses[0] == 0) || (theResponses[1] == 0) || (theResponses[2] == 0) ||
    ...
    opserr << " ReinforcedConcretePlaneStress::ReinforcedConcretePlaneStress - failed to set "
              "appropriate concrete material\n";
    exit(-1);
  }
...
RAFourSteelRCPlaneStress::RAFourSteelRCPlaneStress()
 : NDMaterial(0, ND_TAG_RAFourSteelRCPlaneStress),
   strain_vec(3),
   stress_vec(3),
   tangent_matrix(3, 3)
{
...
double
RAFourSteelRCPlaneStress::getRho(void)
{
  return rho;
}
...
int
RAFourSteelRCPlaneStress::setTrialStrain(const Vector& v)
{
...
///*
int
RAFourSteelRCPlaneStress::setTrialStrain(const Vector& v, const Vector& r)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
int
RAFourSteelRCPlaneStress::setTrialStrainIncr(const Vector& v)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
int
RAFourSteelRCPlaneStress::setTrialStrainIncr(const Vector& v, const Vector& r)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
const Matrix&
RAFourSteelRCPlaneStress::getTangent(void)
{
  return tangent_matrix;
}

const Vector&
RAFourSteelRCPlaneStress::getStress(void)
{
...
const Vector&
RAFourSteelRCPlaneStress ::getStrain()
{
  return strain_vec;
}
...
const Vector&
RAFourSteelRCPlaneStress::getCommittedStress(void)
{
  return stress_vec;
}
...
const Vector&
RAFourSteelRCPlaneStress::getCommittedStrain(void)
{
  return strain_vec;
}
...
int
RAFourSteelRCPlaneStress::commitState(void)
{
  for (int i = 0; i < 6; i++) {
...
int
RAFourSteelRCPlaneStress::revertToLastCommit(void)
{
...
int
RAFourSteelRCPlaneStress::revertToStart(void)
{
  int i;
  for (i = 0; i < 6; i++) {
...
NDMaterial*
RAFourSteelRCPlaneStress::getCopy(void)
{
  /*
  ...
  RAFourSteelRCPlaneStress* theCopy = new RAFourSteelRCPlaneStress(
...
NDMaterial*
RAFourSteelRCPlaneStress::getCopy(const char* type)
{
  /*
    RAFourSteelRCPlaneStress *clone;
    clone = new RAFourSteelRCPlaneStress();
    *clone = *this;
    return clone;
  //*/
  RAFourSteelRCPlaneStress* theModel = new RAFourSteelRCPlaneStress(
      this->getTag(), rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3],
      theMaterial[4], theMaterial[5], angle1, angle2, angle3, angle4, rou1, rou2, rou3, rou4, fpc,
      fy, E0, epsc0);
...
void
RAFourSteelRCPlaneStress::Print(OPS_Stream& s, int flag)
{
...
int
RAFourSteelRCPlaneStress::sendSelf(int commitTag, Channel& theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING RAFourSteelRCPlaneStress::sendSelf() - " << this->getTag()
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (res < 0) {
      opserr << "RAFourSteelRCPlaneStress::sendSelf() - " << this->getTag()
...
int
RAFourSteelRCPlaneStress::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING RAFourSteelRCPlaneStress::recvSelf() - " << this->getTag()
  ...
  if (theMaterial == 0) {
    ...
    if (theMaterial == 0) {
      opserr
          << "RAFourSteelRCPlaneStress::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 6; i++) {
      ...
      if (theMaterial[i] == 0) {
        opserr << "RAFourSteelRCPlaneStress::recvSelf() - Broker could not create NDMaterial of "
                  "class type "
               << matClassTag << endln;
        return -1;
      }
      ...
      if (res < 0) {
        opserr << "RAFourSteelRCPlaneStress::recvSelf() - material " << i
...
int
RAFourSteelRCPlaneStress::determineTrialStress(void)
{
  // Define i, j, k
  int i, j, k;
  ...
  } else // Tstrain[0] != Tstrain[1]
  {
    ...
    if (fabs(Tstress[2]) < 1e-7) {
      ...
      opserr << "RAFourSteelRCPlaneStress::determineTrialStress: Failure to calculate principal "
                "stress direction\n";
