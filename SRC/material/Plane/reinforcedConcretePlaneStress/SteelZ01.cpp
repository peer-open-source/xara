...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_SteelZ01Material)
{
  ...
  if (numRemainingArgs < 5) {
    opserr << "Invalid Args want: uniaxialMaterial SteelZ01 tag? fy? E0? fpc? rou? <ac?> <rc?>"
           << endln;
    return 0;
  }
  ...
  if (numRemainingArgs == 4) {
    ...
    if (OPS_GetDoubleInput(&numRemainingArgs, dData) != 0) {
      ...
      theMaterial =
          new SteelZ01(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5]);
  } else {
...
SteelZ01::SteelZ01(int tag, double FY, double E, double FPC, double ROU, double AC, double RC)
 : UniaxialMaterial(tag, MAT_TAG_SteelZ01), fy(FY), E0(E), fpc(FPC), rou(ROU), ac(AC), rc(RC)
{
  tt1      = 0.0; // temp add
...
SteelZ01::SteelZ01()
 : UniaxialMaterial(0, MAT_TAG_SteelZ01), fy(0.0), E0(0.0), fpc(0.0), rou(0.0), ac(0.0), rc(0.0)
{
  this->revertToStart();
}
...
SteelZ01::getTangent()
{
  ...
  if (Ttangent == 0.0) {
    ...
    opserr << " TreverseBottomStrain[TreverseBottomNum] = "
           << TreverseBottomStrain[TreverseBottomNum] << endln;
...
int
SteelZ01::setTrialStrain(double strain, double strainRate)
{
...
int
SteelZ01::setTrial(double strain, double& stress, double& tangent, double strainRate)
{
  // Reset history variables to last converged state
  TminStrain     = CminStrain;
  TmaxStrain     = CmaxStrain;
...
void
SteelZ01::determineTrialState(double dStrain)
{
  tt1      = Tstrain;
  tt2      = 0.0;
  ttStrain = dStrain;
  ...
  {
    if ((Tstrain > reverseFromTenEnvelopeStrain) ||
        (fabs(Tstrain - reverseFromTenEnvelopeStrain) < 1e-6)) // Back to tension envelope
    {
      reverseLoopSetZero();
      TloadingState = 2;
      tensionEnvelope();
    } else if ((Tstrain < approachToComEnvelopeStrain) ||
               (fabs(Tstrain - approachToComEnvelopeStrain) <
                1e-6)) // Approach to compression envelope
    {
      reverseLoopSetZero();
      TloadingState = 3;
      compressionEnvelope();
  ...
  else if (TloadingState == 5) // In loop reverse from compression envelope
  {
    ...
    opserr << "SteelZ01::determineTrialState -- Improper TloadingState : " << TloadingState
           << " for SteelZ01\n";
  }
...
void
SteelZ01::determineTrialLoop(double dStrain)
{
  ...
  else // create a new inner loop or continue on previous loop
  {
    ...
    if ((TloopPathState == 2 || TloopPathState == 3) && (dStrain > 0)) // reverse up at down path
    {
      ...
      if (TreverseBottomNum > (SIZE - 1)) {
        opserr << " SteelZ01::determineTrialLoop -- overflowed the size of the array storing the "
                  "loop!\n"
    ...
    else if ((TloopPathState == 5 || TloopPathState == 6) &&
             (dStrain < 0)) // reverse down at up path
    {
      TreverseTopNum = TreverseTopNum + 1;
    ...
    else // continue on previous path of the loop
    {
      ...
      else if (TloopPathState == 4 || TloopPathState == 5 || TloopPathState == 6) //up path
      {
        ...
        opserr << " SteelZ01::determineTrialLoop -- improper TloopPathState : " << TloopPathState
               << "\n";
      }
    }
...
void
SteelZ01::determineDownPathPoint()
{
...
SteelZ01::determineUpPathPoint()
{
  ...
  if (TreverseTopStress[TreverseTopNum] < 0.65 * fy) {
    ...
    double tempTwo2      = pow(A, -R) * tempTwo1;
    upPathPointTwoStrain =
        bottomStrain + (upPathPointTwoStress - bottomStress) * (1 + tempTwo2) / E0;
  }
}
...
SteelZ01::downPath()
{
  ...
  if (Tstrain >= downPathPointOneStrain) {
    ...
    TloopPathState = 2;
    double slope2  = (downPathPointTwoStress - downPathPointOneStress) /
                    (downPathPointTwoStrain - downPathPointOneStrain);
    Tstress  = slope2 * (Tstrain - downPathPointOneStrain) + downPathPointOneStress;
    Ttangent = slope2;
  } else {
    TloopPathState = 3;
    double slope3 =
        (bottomStress - downPathPointTwoStress) / (bottomStrain - downPathPointTwoStrain);
...
SteelZ01::upPath()
{
  ...
  if (Tstrain <= upPathPointOneStrain) {
    ...
    TloopPathState = 5;
    slope          = (upPathPointTwoStress - upPathPointOneStress) /
            (upPathPointTwoStrain - upPathPointOneStrain);
...
void
SteelZ01::reverseFromTenEnvelope()
{
  ...
  approachToComEnvelopeStrain =
      reverseFromTenEnvelopeStrain + (-fy - reverseFromTenEnvelopeStress) * (1 + temp2) / E0;
...
void
SteelZ01::reverseFromComEnvelope()
{
  ...
  double temp2         = pow(A, -R) * temp1;
  double tempStrainTwo =
      reverseFromComEnvelopeStrain + (0.65 * fy - reverseFromComEnvelopeStress) * (1 + temp2) / E0;
  ...
  double tempInterOneEnvelopeStrain = (slopeOneTwo * tempStrainOne + fy * (0.91 - 2.0 * B)) /
                                      (slopeOneTwo - 0.25 * B * E0 - 0.02 * E0); //corrected
  double tempInterOneEnvelopeStress = slopeOneTwo * (tempInterOneEnvelopeStrain - tempStrainOne);

  if (tempInterOneEnvelopeStress <
      0.65 * fy) // Intersection occurs at LoopPath5 and tensionEnvelope
  {
  ...
  } else // Intersection occurs at LoopPath6 and tensionEnvelope
  {
    ...
    approachToTenEnvelopeStrain = (slope * tempStrainTwo + fy * (0.26 - 2.0 * B)) /
                                  (slope - 0.25 * B * E0 - 0.02 * E0); //corrected
  ...
  if (approachToTenEnvelopeStrain < epsn) {
    ...
    temp2 = pow(A, -R) * temp1;
    approachToTenEnvelopeStrain =
        reverseFromComEnvelopeStrain + (fn - reverseFromComEnvelopeStress) * (1 + temp2) / E0;
...
int
SteelZ01::sendSelf(int commitTag, Channel& theChannel)
{
  int res = 0;
...
int
SteelZ01::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;
  static Vector data(149);
...
Response*
SteelZ01::setResponse(const char** argv, int argc, OPS_Stream& theOutput)
{
...
void
SteelZ01::Print(OPS_Stream& s, int flag)
{
