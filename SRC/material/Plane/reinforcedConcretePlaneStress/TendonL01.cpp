...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_TendonL01Material)
{
  ...
  if (numRemainingArgs < 6) {
    opserr << "Invalid Args want: uniaxialMaterial TendonL01 tag? fpy? Eps? fpu? rou? epsp? <ac?> "
              "<rc?>"
           << endln;
    return 0;
  }
  ...
  if (numRemainingArgs == 5) {
    if (OPS_GetDoubleInput(&numRemainingArgs, dData) != 0) {
      opserr << "Invalid Args want: uniaxialMaterial TendonL01 tag? fpy? Eps? fpu? rou? epsp? "
                "<ac?> <rc?>"
             << endln;
      return 0;
    } else
...
TendonL01::TendonL01(int tag, double FPY, double E, double FPU, double ROU, double EPSP, double AC,
                     double RC)
...
TendonL01::TendonL01()
 : UniaxialMaterial(0, MAT_TAG_TendonL01),
   fpy(0.0),
   Eps(0.0),
   fpu(0.0),
   rou(0.0),
...
int
TendonL01::setTrialStrain(double strain, double strainRate)
{
...
int
TendonL01::setTrial(double strain, double& stress, double& tangent, double strainRate)
{
  // Reset history variables to last converged state
  TminStrain     = CminStrain;
  TmaxStrain     = CmaxStrain;
...
void
TendonL01::determineTrialState(double dStrain)
{
  tt1      = Tstrain;
  tt2      = 0.0;
  ttStrain = dStrain;
  ...
  else if (TloadingState == 5) // In loop reverse from compression envelope
  {
    ...
    opserr << "TendonL01::determineTrialState -- Improper TloadingState : " << TloadingState
           << " for TendonL01\n";
  }
...
void
TendonL01::determineTrialLoop(double dStrain)
{
  ...
  else // create a new inner loop or continue on previous loop
  {
    ...
    if ((TloopPathState == 2 || TloopPathState == 3) && (dStrain > 0)) // reverse up at down path
    {
      ...
      if (TreverseBottomNum > (SIZE1 - 1)) {
        opserr << " TendonL01::determineTrialLoop -- overflowed the size of the array storing the "
                  "loop!\n"
    ...
    else // continue on previous path of the loop
    {
      ...
      else if (TloopPathState == 4 || TloopPathState == 5 || TloopPathState == 6) //up path
      {
        ...
        opserr << " TendonL01::determineTrialLoop -- improper TloopPathState : " << TloopPathState
               << "\n";
      }
    }
...
void
TendonL01::determineDownPathPoint()
{
  ...
  if (TreverseBottomStress[TreverseBottomNum] > -0.65 * fpy) {
    ...
    downPathPointTwoStrain =
        topStrain + (downPathPointTwoStress - topStress) * (1 + tempTwo2) / Eps;
  }
}

void
TendonL01::determineUpPathPoint()
{
  //double epsy = fy/E0;
  //double fcr = 0.31*sqrt(fpc);
  //if ( rou < 0.0025 ) rou = 0.0025;
  ...
  double tempOne2      = pow(A, -R) * tempOne1;
  upPathPointOneStrain =
      bottomStrain + (upPathPointOneStress - bottomStress) * (1 + tempOne2) / Eps;
  ...
  if (TreverseTopStress[TreverseTopNum] < 0.65 * fpy) {
    ...
    upPathPointTwoStrain =
        bottomStrain + (upPathPointTwoStress - bottomStress) * (1 + tempTwo2) / Eps;
  }
}
...
void
TendonL01::reverseFromTenEnvelope()
{
  ...
  approachToComEnvelopeStrain =
      reverseFromTenEnvelopeStrain + (-fpy - reverseFromTenEnvelopeStress) * (1 + temp2) / Eps;
...
void
TendonL01::reverseFromComEnvelope()
{
  ...
  if (tempInterOneEnvelopeStress <
      0.65 * fpy) // Intersection occurs at LoopPath5 and tensionEnvelope
  {
  ...
  } else // Intersection occurs at LoopPath6 and tensionEnvelope
  {
    ...
    x            = tempStrainTwo;
    fx           = slope * (x - tempStrainTwo) + 0.65 * fpy -
         Epsr / pow((1 + pow(Epsr * x / fpur, 5)), 0.2) * x;
    while (fabs(fx) > 1e-2) {
      x += 0.0001;
      fx = slope * (x - tempStrainTwo) + 0.65 * fpy -
           Epsr / pow((1 + pow(Epsr * x / fpur, 5)), 0.2) * x;
    ...
    approachToTenEnvelopeStress =
        slope * (approachToTenEnvelopeStrain - tempStrainTwo) + 0.65 * fpy;
  }
  ...
  if (approachToTenEnvelopeStrain < epsn) {
    ...
    approachToTenEnvelopeStrain =
        reverseFromComEnvelopeStrain + (fn - reverseFromComEnvelopeStress) * (1 + temp2) / Eps;
...
int
TendonL01::sendSelf(int commitTag, Channel& theChannel)
{
  int res = 0;
...
int
TendonL01::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;
...
Response*
TendonL01::setResponse(const char** argv, int argc, OPS_Stream& theOutput)
{
...
void
TendonL01::Print(OPS_Stream& s, int flag)
{
