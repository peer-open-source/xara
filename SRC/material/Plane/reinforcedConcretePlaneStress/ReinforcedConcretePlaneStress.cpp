...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_ReinforcedConcretePlaneStressMaterial)
{
  if (numReinforcedConcretePlaneStressMaterials == 0) {
    numReinforcedConcretePlaneStressMaterials++;
    opslog << "ReinforcedConcretePlaneStress unaxial material - Written by J.Zhong, Thomas T.C. "
  ...
  if (numRemainingArgs < 14) {
    opserr << "Invalid Args want: NDMaterial ReinforcedConcretePlaneStress matTag? rho? "
  ...
  if (OPS_GetDouble(&numData, &rho) != 0) {
    opserr << "Invalid Arg rho: uniaxialMaterial ReinforcedConcretePlaneStress tag? fy? E0? fpc? "
              "rou? <ac?> <rc?>"
           << endln;
    return 0;
  }
   ...
   tangent_matrix(3, 3)
{
  ...
  // Check allocation
  if (theMaterial[0] == 0) {
    opserr << " ReinforcedConcretePlaneStress::ReinforcedConcretePlaneStress - failed to get a "
              "copy for steel1\n";
    exit(-1);
  }
...
ReinforcedConcretePlaneStress::getCopy(void)
{
  ...
  ReinforcedConcretePlaneStress* theCopy = new ReinforcedConcretePlaneStress(
...
ReinforcedConcretePlaneStress::getCopy(const char* type)
{
  /*
    ...
    *clone = *this;
    return clone;
  //*/
  ReinforcedConcretePlaneStress* theModel = new ReinforcedConcretePlaneStress(
      this->getTag(), rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], angle1,
      angle2, rou1, rou2, fpc, fy, E0, epsc0);
...
ReinforcedConcretePlaneStress::sendSelf(int commitTag, Channel& theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING ReinforcedConcretePlaneStress::sendSelf() - " << this->getTag()
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "ReinforcedConcretePlaneStress::sendSelf() - " << this->getTag()
...
int
ReinforcedConcretePlaneStress::recvSelf(int commitTag, Channel& theChannel,
                                        FEM_ObjectBroker& theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING ReinforcedConcretePlaneStress::recvSelf() - " << this->getTag()
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial*[4];
    if (theMaterial == 0) {
      opserr << "ReinforcedConcretePlaneStress::recvSelf() - Could not allocate UniaxialMaterial* "
                "array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
        opserr << "ReinforcedConcretePlaneStress::recvSelf() - Broker could not create NDMaterial "
                  "of class type "
               << matClassTag << endln;
        return -1;
      }
      ...
      if (res < 0) {
        opserr << "ReinforcedConcretePlaneStress::recvSelf() - material " << i
...
ReinforcedConcretePlaneStress::determineTrialStress(void)
{
  ...
  */
  ...
  if ((epslonOne > 0.0) && (epslonTwo > 0.0)) {
    ...
    opserr
        << "ReinforcedConcretePlaneStress::determineTrialStress: failure to get Hsu/Zhu ratio!\n";
  }
  ...
  */
  ...
  } else // Tstrain[0] != Tstrain[1]
  {
    ...
    if (fabs(Tstress[2]) < 1e-7) {
      ...
      opserr << "ReinforcedConcretePlaneStress::determineTrialStress: Failure to calculate "
