...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_RAFourSteelPCPlaneStressMaterial)
{
  if (numRAFourSteelRCPlaneStressMaterials == 0) {
    numRAFourSteelRCPlaneStressMaterials++;
    opslog << "RAFourSteelRCPlaneStress unaxial material - Written by A.Laskar, Thomas T.C. Hsu "
  ...
  if (numRemainingArgs < 23) {
    opserr << "Want: NDMaterial RAFourSteelPCPlaneStress matTag? rho? UniaxiaMatTag1? "
              "UniaxiaMatTag2? UniaxiaMatTag3? UniaxiaMatTag4? UniaxiaMatTag5? UniaxiaMatTag6? "
              "angle1? angle2? angle3? angle4? rou1? rou2? rou3? rou4? pstrain1? pstrain2? fpc? "
              "fy1? fy2? E0? epsc0?\n";
    return 0;
  }
...
RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress(int tag,
                                                   ...
                                                   double E,
                                                   double EPSC0)
 : NDMaterial(tag, ND_TAG_RAFourSteelPCPlaneStress),
   rho(RHO),
   angle1(ANGLE1),
   angle2(ANGLE2),
   angle3(ANGLE3),
   ...
   tangent_matrix(3, 3)
{
  ...
  if (theMaterial == 0) {
    opserr << " RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress - failed to allocate material "
              "array\n";
    exit(-1);
  }
  ...
  // Check allocation
  if (theMaterial[0] == 0) {
    opserr << " RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress - failed to get a copy for "
              "tendon1\n";
    exit(-1);
  }
  ...
  if ((theResponses[0] == 0) || (theResponses[1] == 0) || (theResponses[2] == 0) ||
    ...
    opserr << " RAFourSteelPCPLaneStress::RAFourSteelPCPlaneStress - failed to set appropriate "
              "materials tag:"
           << tag << endln;
    exit(-1);
  }
...
RAFourSteelPCPlaneStress::RAFourSteelPCPlaneStress()
 : NDMaterial(0, ND_TAG_RAFourSteelPCPlaneStress),
   strain_vec(3),
   stress_vec(3),
   tangent_matrix(3, 3)
{
...
double
RAFourSteelPCPlaneStress::getRho(void)
{
  return rho;
}
...
int
RAFourSteelPCPlaneStress::setTrialStrain(const Vector& v)
{
...
///*
int
RAFourSteelPCPlaneStress::setTrialStrain(const Vector& v, const Vector& r)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
int
RAFourSteelPCPlaneStress::setTrialStrainIncr(const Vector& v)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
int
RAFourSteelPCPlaneStress::setTrialStrainIncr(const Vector& v, const Vector& r)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
const Matrix&
RAFourSteelPCPlaneStress::getTangent(void)
{
  return tangent_matrix;
}

const Vector&
RAFourSteelPCPlaneStress::getStress(void)
{
...
const Vector&
RAFourSteelPCPlaneStress ::getStrain()
{
  return strain_vec;
}
...
const Vector&
RAFourSteelPCPlaneStress::getCommittedStress(void)
{
  return stress_vec;
}
...
const Vector&
RAFourSteelPCPlaneStress::getCommittedStrain(void)
{
  return strain_vec;
}
...
int
RAFourSteelPCPlaneStress::commitState(void)
{
  for (int i = 0; i < 6; i++) {
...
int
RAFourSteelPCPlaneStress::revertToLastCommit(void)
{
...
int
RAFourSteelPCPlaneStress::revertToStart(void)
{
  int i;
  for (i = 0; i < 6; i++) {
...
NDMaterial*
RAFourSteelPCPlaneStress::getCopy(void)
{
  /*
  ...
  RAFourSteelPCPlaneStress* theCopy = new RAFourSteelPCPlaneStress(
      this->getTag(), rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3],
      theMaterial[4], theMaterial[5], angle1, angle2, angle3, angle4, rou1, rou2, rou3, rou4,
      pstrain1, pstrain2, fpc, fy1, fy2, E0, epsc0);
...
NDMaterial*
RAFourSteelPCPlaneStress::getCopy(const char* type)
{
  /*
    RAFourSteelPCPlaneStress *clone;
    clone = new RAFourSteelPCPlaneStress();
    *clone = *this;
    return clone;
  //*/
  RAFourSteelPCPlaneStress* theModel = new RAFourSteelPCPlaneStress(
      this->getTag(), rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3],
      theMaterial[4], theMaterial[5], angle1, angle2, angle3, angle4, rou1, rou2, rou3, rou4,
      pstrain1, pstrain2, fpc, fy1, fy2, E0, epsc0);
...
void
RAFourSteelPCPlaneStress::Print(OPS_Stream& s, int flag)
{
...
int
RAFourSteelPCPlaneStress::sendSelf(int commitTag, Channel& theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING RAFourSteelPCPlaneStress::sendSelf() - " << this->getTag()
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (res < 0) {
      opserr << "RAFourSteelPCPlaneStress::sendSelf() - " << this->getTag()
...
int
RAFourSteelPCPlaneStress::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING RAFourSteelPCPlaneStress::recvSelf() - " << this->getTag()
  ...
  if (theMaterial == 0) {
    ...
    if (theMaterial == 0) {
      opserr
          << "RAFourSteelPCPlaneStress::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 6; i++) {
      ...
      if (theMaterial[i] == 0) {
        opserr << "RAFourSteelPCPlaneStress::recvSelf() - Broker could not create NDMaterial of "
                  "class type "
               << matClassTag << endln;
        return -1;
      }
      ...
      if (res < 0) {
        opserr << "RAFourSteelPCPlaneStress::recvSelf() - material " << i
...
int
RAFourSteelPCPlaneStress::determineTrialStress(void)
{
  // Define i, j, k
  int i, j, k;
  ...
  double stressS1, stressS2, stressS3,
      stressS4; // stress of steel layers in 1, 2, 3 and 4 directions
  ...
  Tstrain[2] = 0.5 * strain_vec(2) - pstrain1 * 2.0 * cos(citaS1) * sin(citaS1) -
               pstrain2 * 2.0 * cos(citaS2) * sin(citaS2);
  ...
  Tstress[2] =
      cos(citaR) * sin(citaR) * cigmaOneC - cos(citaR) * sin(citaR) * cigmaTwoC +
  ...
  {
    temp_citaP = 0.5 * atan(fabs(2.0 * 1000000.0 * Tstress[2] /
    ...
    if (fabs(Tstress[2]) < 1e-7) {
      ...
      opserr << "RAFourSteelPCPlaneStress::determineTrialStress: Failure to calculate principal "
                "stress direction\n";
