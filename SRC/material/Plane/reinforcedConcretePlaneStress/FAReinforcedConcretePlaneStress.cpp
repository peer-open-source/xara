...
// Created: 2004.5
//
// Description: This file contains the class definition for  FAReinforcedConcretePlaneStress
//
...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_FAReinforcedConcretePlaneStressMaterial)
{
  if (numFAReinforcedConcretePlaneStressMaterials == 0) {
    numFAReinforcedConcretePlaneStressMaterials++;
    opslog << "FAReinforcedConcretePlaneStress unaxial material - Written by J.Zhong, Thomas T.C. "
  ...
  if (numRemainingArgs < 14) {
    opserr << "Invalid Args want: NDMaterial FAReinforcedConcretePlaneStress matTag? rho? "
              "UniaxiaMatTag1? UniaxiaMatTag2? UniaxiaMatTag3? UniaxiaMatTag4? angle1? angle2? "
              "rou1? rou2? fpc? fy? E0? epsc0?>\n";
    return 0;
  }
  ...
  if (OPS_GetDouble(&numData, &rho) != 0) {
    opserr << "Invalid Arg rho: uniaxialMaterial FAReinforcedConcretePlaneStress tag:" << tag
           << endln;
    return 0;
  }
  ...
  if (OPS_GetInt(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial FAReinforcedConcretePlaneStress tag: " << tag
           << endln;
    return 0;
  }
...
double
FAReinforcedConcretePlaneStress::getRho()
{
  return rho;
}
...
const Matrix&
FAReinforcedConcretePlaneStress::getTangent()
{
  return tangent_matrix;
}

const Vector&
FAReinforcedConcretePlaneStress::getStress()
{
...
const Vector&
FAReinforcedConcretePlaneStress::getCommittedStress()
{
  return stress_vec;
}
...
const Vector&
FAReinforcedConcretePlaneStress::getCommittedStrain()
{
  return strain_vec;
}
...
int
FAReinforcedConcretePlaneStress::commitState()
{
  for (int i = 0; i < 4; i++) {
...
int
FAReinforcedConcretePlaneStress::revertToLastCommit()
{
...
int
FAReinforcedConcretePlaneStress::revertToStart()
{
  int i;
  for (i = 0; i < 4; i++) {
...
NDMaterial*
FAReinforcedConcretePlaneStress::getCopy()
{
  /*
  ...
  FAReinforcedConcretePlaneStress* theCopy = new FAReinforcedConcretePlaneStress(this->getTag(),
                                                                                 rho,
                                                                                 theMaterial[0],
                                                                                 theMaterial[1],
                                                                                 theMaterial[2],
...
FAReinforcedConcretePlaneStress::getCopy(const char* type)
{
  /*
	...
	*clone = *this;
	return clone;
	//*/
  FAReinforcedConcretePlaneStress* theModel = new FAReinforcedConcretePlaneStress(
      this->getTag(), rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], angle1,
      angle2, rou1, rou2, fpc, fy, E0, epsc0);
...
FAReinforcedConcretePlaneStress::sendSelf(int commitTag, Channel& theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING FAReinforcedConcretePlaneStress::sendSelf() - " << this->getTag()
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "FAReinforcedConcretePlaneStress::sendSelf() - " << this->getTag()
...
int
FAReinforcedConcretePlaneStress::recvSelf(int commitTag, Channel& theChannel,
                                          FEM_ObjectBroker& theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING FAReinforcedConcretePlaneStress::recvSelf() - " << this->getTag()
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial*[4];
    if (theMaterial == 0) {
      opserr << "FAReinforcedConcretePlaneStress::recvSelf() - Could not allocate "
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
        opserr << "FAReinforcedConcretePlaneStress::recvSelf() - Broker could not create "
      ...
      if (res < 0) {
        opserr << "FAReinforcedConcretePlaneStress::recvSelf() - material " << i
...
int
FAReinforcedConcretePlaneStress::determineTrialStress()
{
  ...
  } else // Tstrain[0] != Tstrain[1]
  {
    ...
    if (fabs(Tstrain[2]) < 1e-7) {
      ...
      opserr
          << "FAReinforcedConcretePlaneStress::determineTrialStress: Failure to calculate citaR\n";
...
{
  // Trial principal stress direction, obtained from input
  double citaIn = inputAngle;
  ...
  if ((epslonOne > 0.0) && (epslonTwo > 0.0)) {
    ...
    opserr << "FAReinforcedConcretePlaneStress::getPrincipalStressAngle: failure to get Hsu/Zhu "
              "ratio!\n";
  }
  ...
  if (v12 * v21 == 1.0) {
    opserr
        << "FAReinforcedConcretePlaneStress::getPrincipalStressAngle: failure to get matrix [V]!\n";
  ...
  } else // Tstrain[0] != Tstrain[1]
  {
    ...
    if (fabs(Tstress[2]) < 1e-7) {
      ...
      opserr << "FAReinforcedConcretePlaneStress::getPrincipalStressAngle: Failure to calculate "
