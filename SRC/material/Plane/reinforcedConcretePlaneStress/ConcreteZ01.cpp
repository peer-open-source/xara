...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_ConcreteZ01Material)
{
...
ConcreteZ01::ConcreteZ01(int tag, double FPC, double EPSC0)
...
ConcreteZ01::ConcreteZ01() : UniaxialMaterial(0, MAT_TAG_ConcreteZ01), fpc(0.0), epsc0(0.0)
{
  // Set trial values
  this->revertToStart();
}
...
int
ConcreteZ01::setTrialStrain(double x, double k, double Dfactor, double ITAP, double EPSLONTP,
                            double strain, double strainRate)
{
  X        = x;
  K        = k;
  D        = Dfactor;
...
int
ConcreteZ01::setTrialStrain(double strain, double strainRate)
{
  /* FMK
   X = x;
   K = k;
...
int
ConcreteZ01::setTrial(double strain, double& stress, double& tangent, double strainRate)
{
  /* FMK
   X = x;
   K = k;
...
double
ConcreteZ01::getStrain()
{
  return Tstrain;
}

double
ConcreteZ01::getStress()
{
  return Tstress;
}

double
ConcreteZ01::getTangent()
{
  return Ttangent;
}

double
ConcreteZ01::getSecant()
{
  double TSecant;
  if (Tstrain == 0.0) {
...
int
ConcreteZ01::revertToLastCommit()
{
...
void
ConcreteZ01::determineTrialState(double dStrain)
{
  double Ec0 = 2.0 * fpc / epsc0;
  if (TloadingState == 0) {
    envelope();
  ...
  if (TloadingState == 1) {
    ...
    opserr << " ConcreteZ01::determineTrialState -- impropter TloadingState: " << TloadingState
           << "\n";
...
ConcreteZ01::pathFive()
{
  ...
  if (reloadPath == 1) {
    ...
    opserr << " ConcreteZ01::pathFive -- improper reloadPath : " << reloadPath << "\n";
  }
}


void
ConcreteZ01::pathSeven()
{
...
void
ConcreteZ01::getApproachFiveToComStrain()
{
  approachFiveToComStrain = 0.0;
  ...
  if (tempB * tempB - 4.0 * tempC < 0.0) {
    opserr << " ConcreteZ01::getApproachFiveToComStrain -- can not get root of equation: sqrt(x) "
              "x<0! \n";
  }
  ...
  } else // intersection occurs at descending branch
  {
    ...
    fxnn = zeta * D * fpc -
           zeta * D * fpc * pow(xnn / (zeta * epsc0) - 1.0, X) / pow(4.0 / zeta - 1.0, X) -
           tempK * xnn - b;

    int counter = 0;

    if ((tempK * zeta * epsc0 + b) < zeta * D * fpc) {
      opserr << " ConcreteZ01::getApproachFiveToComStrain -- No intersection of reloading path "
    ...
    while ((fabs(fxnn) > 1e-4) && (counter < 50)) {
      xn   = xnn;
      fxnp = -(X)*D * fpc * pow(xnn / (zeta * epsc0) - 1, -1 + X) / pow(4 / zeta - 1.0, X) / epsc0 -
             tempK;
      xnn  = xn - fxnn / fxnp;
      fxnn = zeta * D * fpc -
             zeta * D * fpc * pow((xnn / (zeta * epsc0) - 1.0), X) / pow(4.0 / zeta - 1.0, X) -
...
void
ConcreteZ01::getApproachSixToComStrain()
{
  approachSixToComStrain = 0.0;
  ...
  if (epslonTP <= 0.0) {
    ...
    if (TloadingState == 1) {
      ...
      if (Ttangent == 0.0) // at the end platum part of descending branch FMK CHANGED FROM = 0.0
      {
        ...
        tempRatio = Tstrain / (zeta * epsc0);
        PD        = -D * 1160.0 * sqrt(-fpc) / itap * pow((1 + 400.0 * epslonTP / itap), -1.5) *
             (1.0 - (tempRatio - 1) / pow(4.0 / zeta - 1.0, 3.0) *
                        (1.0 - 12.0 / zeta + (4.0 / zeta + 1.0) * tempRatio));
        // 1160, 400
      }
    } else {
      PD = 0.0;
...
int
ConcreteZ01::sendSelf(int commitTag, Channel& theChannel)
{
  int res = 0;
...
int
ConcreteZ01::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;
  static Vector data(21);
...
Response*
ConcreteZ01::setResponse(const char** argv, int argc, OPS_Stream& theOutput)
{
