...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_PrestressedConcretePlaneStressMaterial)
{
  if (numPrestressedConcretePlaneStressMaterials == 0) {
    numPrestressedConcretePlaneStressMaterials++;
    opslog << "PrestressedConcretePlaneStress unaxial material - Written by A.Laskar, Thomas T.C. "
  ...
  if (numRemainingArgs < 16) {
    opserr << "Want: nDMaterial PrestressedConcretePlaneStress matTag? rho? UniaxiaMatTag1? "
  ...
  if (OPS_GetDouble(&numData, &rho) != 0) {
    opserr << "Invalid Arg rho: uniaxialMaterial PrestressedConcretePlaneStress tag " << tag
           << endln;
    return 0;
  }
   ...
   tangent_matrix(3, 3)
{
  ...
  if (theMaterial == 0) {
    opserr << " PrestressedConcretePlaneStress::PrestressedConcretePlaneStress - failed allocate "
              "material array\n";
    exit(-1);
  }
  ...
  // Check allocation
  if (theMaterial[0] == 0) {
    opserr << " PrestressedConcretePlaneStress::PrestressedConcretePlaneStress - failed to get a "
              "copy for tendon1\n";
    exit(-1);
  }
  ...
  if ((theResponses[0] == 0) || (theResponses[1] == 0) || (theResponses[2] == 0) ||
    ...
    opserr << " PrestressedConcretePlaneStress::PrestressedConcretePlaneStress - failed to set "
...
PrestressedConcretePlaneStress::PrestressedConcretePlaneStress()
 : NDMaterial(0, ND_TAG_PrestressedConcretePlaneStress),
   strain_vec(3),
   stress_vec(3),
   tangent_matrix(3, 3)
{
...
double
PrestressedConcretePlaneStress::getRho(void)
{
  return rho;
}
...
int
PrestressedConcretePlaneStress::setTrialStrain(const Vector& v)
{
...
///*
int
PrestressedConcretePlaneStress::setTrialStrain(const Vector& v, const Vector& r)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
int
PrestressedConcretePlaneStress::setTrialStrainIncr(const Vector& v)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
int
PrestressedConcretePlaneStress::setTrialStrainIncr(const Vector& v, const Vector& r)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
const Matrix&
PrestressedConcretePlaneStress::getTangent(void)
{
  return tangent_matrix;
}

const Vector&
PrestressedConcretePlaneStress::getStress(void)
{
...
const Vector&
PrestressedConcretePlaneStress ::getStrain()
{
  return strain_vec;
}
...
const Vector&
PrestressedConcretePlaneStress::getCommittedStress(void)
{
  return stress_vec;
}
...
const Vector&
PrestressedConcretePlaneStress::getCommittedStrain(void)
{
  return strain_vec;
}
...
int
PrestressedConcretePlaneStress::commitState(void)
{
  for (int i = 0; i < 4; i++) {
...
int
PrestressedConcretePlaneStress::revertToLastCommit(void)
{
...
int
PrestressedConcretePlaneStress::revertToStart(void)
{
  int i;
  for (i = 0; i < 4; i++) {
...
NDMaterial*
PrestressedConcretePlaneStress::getCopy(void)
{
  /*
  ...
  PrestressedConcretePlaneStress* theCopy = new PrestressedConcretePlaneStress(
...
NDMaterial*
PrestressedConcretePlaneStress::getCopy(const char* type)
{
  /*
	PrestressedConcretePlaneStress *clone;
	clone = new PrestressedConcretePlaneStress();
	*clone = *this;
	return clone;
	//*/
  PrestressedConcretePlaneStress* theModel = new PrestressedConcretePlaneStress(
      this->getTag(), rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], angle1,
      angle2, rou1, rou2, pstrain, fpc, fy1, fy2, E0, epsc0);
...
void
PrestressedConcretePlaneStress::Print(OPS_Stream& s, int flag)
{
...
int
PrestressedConcretePlaneStress::sendSelf(int commitTag, Channel& theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING PrestressedConcretePlaneStress::sendSelf() - " << this->getTag()
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "PrestressedConcretePlaneStress::sendSelf() - " << this->getTag()
...
int
PrestressedConcretePlaneStress::recvSelf(int commitTag, Channel& theChannel,
                                         FEM_ObjectBroker& theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING PrestressedConcretePlaneStress::recvSelf() - " << this->getTag()
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial*[4];
    if (theMaterial == 0) {
      opserr << "PrestressedConcretePlaneStress::recvSelf() - Could not allocate UniaxialMaterial* "
                "array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
        opserr << "PrestressedConcretePlaneStress::recvSelf() - Broker could not create NDMaterial "
                  "of class type "
               << matClassTag << endln;
        return -1;
      }
      ...
      if (res < 0) {
        opserr << "PrestressedConcretePlaneStress::recvSelf() - material " << i
...
int
PrestressedConcretePlaneStress::determineTrialStress(void)
{
  // Define i, j, k
  int i, j, k;
  ...
  } else // Tstrain[0] != Tstrain[1]
  {
    ...
    if (fabs(Tstrain[2]) < 1e-7) {
      ...
      opserr
          << "PrestressedConcretePlaneStress::determineTrialStress: Failure to calculate citaR\n";
  ...
  if ((epslonOne > 0.0) && (epslonTwo > 0.0)) {
    ...
    opserr
        << "PrestressedConcretePlaneStress::determineTrialStress: failure to get Hsu/Zhu ratio!\n";
  }
  ...
  } else // Tstrain[0] != Tstrain[1]
  {
    ...
    if (fabs(Tstress[2]) < 1e-7) {
      ...
      opserr << "PrestressedConcretePlaneStress::determineTrialStress: Failure to calculate "
