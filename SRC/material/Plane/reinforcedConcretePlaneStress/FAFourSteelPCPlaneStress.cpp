...
OPS_Export void*
OPS_ADD_RUNTIME_VPV(OPS_FAFourSteelPCPlaneStressMaterial)
{
  if (numFAFourSteelPCPlaneStressMaterials == 0) {
    numFAFourSteelPCPlaneStressMaterials++;
    opslog << "FAFourSteelPCPlaneStress unaxial material - Written by A.Laskar, Thomas T.C. Hsu "
  ...
  if (numRemainingArgs < 23) {
    opserr
        << "Want: NDMaterial FAFourSteelPCPlaneStress matTag? rho? UniaxiaMatTag1? UniaxiaMatTag2? UniaxiaMatTag3? Un\
...
FAFourSteelPCPlaneStress::FAFourSteelPCPlaneStress(int tag,
                                                   ...
                                                   double E,
                                                   double EPSC0)
 : NDMaterial(tag, ND_TAG_FAFourSteelPCPlaneStress),
   rho(RHO),
   angle1(ANGLE1),
   angle2(ANGLE2),
   angle3(ANGLE3),
   ...
   tangent_matrix(3, 3)
{
  ...
  if (theMaterial == 0) {
    opserr << " FAFourSteelPCPlaneStress::FAFourSteelPCPlaneStress - failed to allocate material "
              "array\n";
    exit(-1);
  }
  ...
  // Check allocation
  if (theMaterial[0] == 0) {
    opserr << " FAFourSteelPCPlaneStress::FAFourSteelPCPlaneStress - failed to get a copy for "
              "tendon1\n";
    exit(-1);
  }
  ...
  if ((theResponses[0] == 0) || (theResponses[1] == 0) || (theResponses[2] == 0) ||
    ...
    opserr << " FAFourSteelPCPLaneStress::FAFourSteelPCPlaneStress - failed to set appropriate "
              "materials tag:"
           << tag << endln;
    exit(-1);
  }
...
FAFourSteelPCPlaneStress::FAFourSteelPCPlaneStress()
 : NDMaterial(0, ND_TAG_FAFourSteelPCPlaneStress),
   strain_vec(3),
   stress_vec(3),
   tangent_matrix(3, 3)
{
...
double
FAFourSteelPCPlaneStress::getRho(void)
{
  return rho;
}
...
int
FAFourSteelPCPlaneStress::setTrialStrain(const Vector& v)
{
...
///*
int
FAFourSteelPCPlaneStress::setTrialStrain(const Vector& v, const Vector& r)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
int
FAFourSteelPCPlaneStress::setTrialStrainIncr(const Vector& v)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
int
FAFourSteelPCPlaneStress::setTrialStrainIncr(const Vector& v, const Vector& r)
{
  // Set values for strain_vec
  strain_vec(0) = v(0);
  strain_vec(1) = v(1);
...
const Matrix&
FAFourSteelPCPlaneStress::getTangent(void)
{
  return tangent_matrix;
}

const Vector&
FAFourSteelPCPlaneStress::getStress(void)
{
...
const Vector&
FAFourSteelPCPlaneStress ::getStrain()
{
  return strain_vec;
}
...
const Vector&
FAFourSteelPCPlaneStress::getCommittedStress(void)
{
  return stress_vec;
}
...
const Vector&
FAFourSteelPCPlaneStress::getCommittedStrain(void)
{
  return strain_vec;
}
...
int
FAFourSteelPCPlaneStress::commitState(void)
{
  int i;
  for (i = 0; i < 6; i++) {
...
int
FAFourSteelPCPlaneStress::revertToLastCommit(void)
{
...
int
FAFourSteelPCPlaneStress::revertToStart(void)
{
  int i;
  for (i = 0; i < 6; i++) {
...
NDMaterial*
FAFourSteelPCPlaneStress::getCopy(void)
{
  /*
  ...
  FAFourSteelPCPlaneStress* theCopy = new FAFourSteelPCPlaneStress(
...
NDMaterial*
FAFourSteelPCPlaneStress::getCopy(const char* type)
{
  /*
	FAFourSteelPCPlaneStress *clone;
	clone = new FAFourSteelPCPlaneStress();
	*clone = *this;
	return clone;
	//*/
  FAFourSteelPCPlaneStress* theModel = new FAFourSteelPCPlaneStress(
      this->getTag(), rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3],
      theMaterial[4], theMaterial[5], angle1, angle2, angle3, angle4, rou1, rou2, rou3, rou4,
      pstrain1, pstrain2, fpc, fy1, fy2, E0, epsc0);
...
void
FAFourSteelPCPlaneStress::Print(OPS_Stream& s, int flag)
{
...
int
FAFourSteelPCPlaneStress::sendSelf(int commitTag, Channel& theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FAFourSteelPCPlaneStress::sendSelf() - " << this->getTag()
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (res < 0) {
      opserr << "FAFourSteelPCPlaneStress::sendSelf() - " << this->getTag()
...
int
FAFourSteelPCPlaneStress::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FAFourSteelPCPlaneStress::recvSelf() - " << this->getTag()
  ...
  if (theMaterial == 0) {
    ...
    if (theMaterial == 0) {
      opserr
          << "FAFourSteelPCPlaneStress::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 6; i++) {
      ...
      if (theMaterial[i] == 0) {
        opserr << "FAFourSteelPCPlaneStress::recvSelf() - Broker could not create NDMaterial of "
                  "class type "
               << matClassTag << endln;
        return -1;
      }
      ...
      if (res < 0) {
        opserr << "FAFourSteelPCPlaneStress::recvSelf() - material " << i
...
int
FAFourSteelPCPlaneStress::determineTrialStress(void)
{
  ...
  Tstrain[2] = 0.5 * strain_vec(2) - pstrain1 * 2.0 * cos(citaS1) * sin(citaS1) -
               pstrain2 * 2.0 * cos(citaS1) * sin(citaS2);
...
double
FAFourSteelPCPlaneStress::getAngleError(double inputCita)
{
  double pi = 3.14159265359;
  double outputCita;
...
double
FAFourSteelPCPlaneStress::getPrincipalStressAngle(double inputAngle)
{
