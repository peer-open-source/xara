...
J2BeamFiber2d::J2BeamFiber2d(int tag, double e, double g, double sy, double hi, double hk)
...
const Matrix&
J2BeamFiber2d::getInitialTangent()
{
...
int
J2BeamFiber2d::revertToLastCommit()
{
  epsPn1[0] = epsPn[0];
  epsPn1[1] = epsPn[1];
...
{
  J2BeamFiber2d* theCopy = new J2BeamFiber2d(this->getTag(), E, nu, sigmaY, Hiso, Hkin);
...
const char*
J2BeamFiber2d::getType() const
{
  return "BeamFiber2d";
}

int
J2BeamFiber2d::getOrder() const
{
  return 2;
}
...
J2BeamFiber2d::commitSensitivity(const Vector& depsdh, int gradIndex, int numGrads)
{
  ...
  if (F <= -100 * DBL_EPSILON) {
    ...
    dalphadh +=
        dx(2) * root23 * q + dg * root23 * (xsi[0] * two3 * dx(0) + xsi[1] * 2.0 * dx(1)) / q;
    depsPdh[0] += dx(2) * two3 * xsi[0] + dg * two3 * dx(0);
    depsPdh[1] += dx(2) * 2.0 * xsi[1] + dg * 2.0 * dx(1);
...
int
J2BeamFiber2d::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  int res = 0;
...
int
J2BeamFiber2d::setParameter(const char** argv, int argc, Parameter& param)
{
