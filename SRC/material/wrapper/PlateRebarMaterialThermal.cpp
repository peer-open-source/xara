...
PlateRebarMaterialThermal::PlateRebarMaterialThermal()
 : NDMaterial(0, ND_TAG_PlateRebarMaterialThermal), strain(5)
{
}


//full constructor
PlateRebarMaterialThermal::PlateRebarMaterialThermal(int tag, UniaxialMaterial& uniMat, double ang)
 : NDMaterial(tag, ND_TAG_PlateRebarMaterialThermal), strain(5), angle(ang), temperature(0)
{
...
PlateRebarMaterialThermal::getCopy()
{
  ...
  clone = new PlateRebarMaterialThermal(this->getTag(), *theMat,
                                        angle); //make the copy
...
PlateRebarMaterialThermal::setTrialStrain(const Vector& strainFromElement)
{
  ...
  return theMat->setTrialStrain(strain(0) * c * c + strain(1) * s * s + strain(2) * c * s,
                                temperature, 0);
}
...
PlateRebarMaterialThermal::getThermalTangentAndElongation(double& TempT, double& ET, double& Elong)
{
  ...
  theMat->getVariable(
      "ElongTangent",
      iData); //Actually here update initial tangent and thermalElongation corresponding  to current temperature
  tData             = iData.getData();
  ET                = tData(1);
...
PlateRebarMaterialThermal::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  ...
  if (theMat->getClassTag() != matClassTag) {
    ...
    if (theMat == 0) {
      opserr << "PlateRebarMaterialThermal::recvSelf() - failed to get a material of type: "
             << matClassTag << endln;
      return -1;
    }
  }
...
int
PlateRebarMaterialThermal::getResponse(int responseID, Information& matInfo)
{
  static Vector tempData(2);
  static Information infoData(tempData);
  switch (responseID) {
