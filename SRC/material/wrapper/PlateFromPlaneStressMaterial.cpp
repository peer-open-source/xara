...
void*
OPS_ADD_RUNTIME_VPV(OPS_PlateFromPlaneStressMaterial)
{
...
PlateFromPlaneStressMaterial::PlateFromPlaneStressMaterial()
 : NDMaterial(0, ND_TAG_PlateFromPlaneStressMaterial), strain(5)
{
}


//full constructor
PlateFromPlaneStressMaterial::PlateFromPlaneStressMaterial(int tag, NDMaterial& ndMat, double g)
 : NDMaterial(tag, ND_TAG_PlateFromPlaneStressMaterial), strain(5), gmod(g)
{
...
int
PlateFromPlaneStressMaterial::recvSelf(int commitTag,
                                       Channel& theChannel,
                                       FEM_ObjectBroker& theBroker)
{
  ...
  if (theMat->getClassTag() != matClassTag) {
    ...
    if (theMat == 0) {
      opserr << "PlateFromPlaneStressMaterial::recvSelf() - failed to get a material of type: "
             << matClassTag << endln;
      return -1;
    }
  }
...
PlateFromPlaneStressMaterial::setResponse(const char** argv, int argc, OPS_Stream& output)
{
  if (strcmp(argv[0], "Tangent") == 0 || strcmp(argv[0], "tangent") == 0 ||
    ...
    return NDMaterial::setResponse(argv,
                                   argc,
                                   output); // for stresses/strains, get response from NDMaterial
  }
  ...
  if (theResponse == 0)
    return NDMaterial::setResponse(
        argv,
        argc,
        output); // not implemented, get default (zero) response from NDMaterial
