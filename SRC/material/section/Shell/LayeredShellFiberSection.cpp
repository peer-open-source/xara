...
LayeredShellFiberSection::LayeredShellFiberSection()
 : SectionForceDeformation(0, SEC_TAG_LayeredShellFiberSection), strainResultant(8), nLayers(0)
{
}
...
LayeredShellFiberSection::LayeredShellFiberSection(int tag,
                                                   ...
                                                   double* thickness,
                                                   NDMaterial** fibers)
 : SectionForceDeformation(tag, SEC_TAG_LayeredShellFiberSection), strainResultant(8)
{
...
Response*
LayeredShellFiberSection::setResponse(const char** argv, int argc, OPS_Stream& output)
{
...
LayeredShellFiberSection::sendSelf(int commitTag, Channel& theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING LayeredShellFiberSection::sendSelf() - " << this->getTag()
...
LayeredShellFiberSection::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING LayeredShellFiberSection::recvSelf() - " << this->getTag()
  ...
  if (nLayers > 0) {
    ...
    for (i = 0; i < nLayers; i++) {
      ...
      if (res < 0) {
        opserr << "LayeredShellFiberSection::recvSelf() - material " << i
