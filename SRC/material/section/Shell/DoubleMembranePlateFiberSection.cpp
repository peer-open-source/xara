...
DoubleMembranePlateFiberSection::DoubleMembranePlateFiberSection()
 : SectionForceDeformation(0, SEC_TAG_DoubleMembranePlateFiberSection), strainResultant(8)
{
...
//full constructor
DoubleMembranePlateFiberSection::DoubleMembranePlateFiberSection(int tag,
                                                                 double thickness,
                                                                 double distance,
                                                                 NDMaterial& Afiber)
 : SectionForceDeformation(tag, SEC_TAG_DoubleMembranePlateFiberSection), strainResultant(8)
{
  this->h = thickness;
  this->d = distance;
...
DoubleMembranePlateFiberSection::getCopy()
{
  ...
  clone = new DoubleMembranePlateFiberSection(this->getTag(), this->h, this->d,
...
const Vector&
DoubleMembranePlateFiberSection::getStressResultant()
{
...
DoubleMembranePlateFiberSection::sendSelf(int commitTag, Channel& theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING DoubleMembranePlateFiberSection::sendSelf() - " << this->getTag()
           << " failed to send ID\n";
...
int
DoubleMembranePlateFiberSection::recvSelf(int commitTag, Channel& theChannel,
                                          FEM_ObjectBroker& theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING DoubleMembranePlateFiberSection::recvSelf() - " << this->getTag()
  ...
  if (theFibers[0] == 0) {
    for (i = 0; i < numFibers; i++) {
      ...
      if (res < 0) {
        opserr << "DoubleMembranePlateFiber::recvSelf() - material " << i
  ...
  // Number of materials is the same, receive materials into current space
    ...
    for (i = 0; i < numFibers; i++) {
      ...
      res += theFibers[i]->recvSelf(commitTag, theChannel, theBroker);
      if (res < 0) {
        opserr << "DoubleMembranePlateFiberSection::recvSelf() - material " << i
               << ", failed to recv itself\n";
        return res;
      }
    }
...
Response*
DoubleMembranePlateFiberSection::setResponse(const char** argv, int argc, OPS_Stream& output)
{
