...
const double MembranePlateFiberSectionThermal::sg[] = {-1, -0.65465367, 0, 0.65465367, 1};

const double MembranePlateFiberSectionThermal::wg[] = {0.1, 0.5444444444, 0.7111111111,
                                                       0.5444444444, 0.1};
...
MembranePlateFiberSectionThermal::MembranePlateFiberSectionThermal()
 : SectionForceDeformation(0, SEC_TAG_MembranePlateFiberSectionThermal), strainResultant(8)
{
  for (int i = 0; i < 5; i++)
    theFibers[i] = 0;
  this->countnGauss = 0;
...
MembranePlateFiberSectionThermal::MembranePlateFiberSectionThermal(int tag,
                                                                   double thickness,
                                                                   NDMaterial& Afiber)
 : SectionForceDeformation(tag, SEC_TAG_MembranePlateFiberSectionThermal), strainResultant(8)
{
  this->h = thickness;
...
int
MembranePlateFiberSectionThermal ::setTrialSectionDeformation(
...
MembranePlateFiberSectionThermal::getTemperatureStress(const Vector& dataMixed)
{
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (fabs(dataTempe[1]) <= 1e-10 && fabs(dataTempe[17]) <= 1e-10) {
      ...
      if (yi < dataTempe[1]) {
        opserr << "MembranePlateFiberSectionThermal::setTrialSectionDeformationup -- fiber loc is "
                  "out of the section";
      } else if (yi <= dataTempe[3]) {
        FiberTemperature = dataTempe[0] - (dataTempe[1] - yi) * (dataTempe[0] - dataTempe[2]) /
                                              (dataTempe[1] - dataTempe[3]);
      } else if (yi <= dataTempe[5]) {
        FiberTemperature = dataTempe[2] - (dataTempe[3] - yi) * (dataTempe[2] - dataTempe[4]) /
                                              (dataTempe[3] - dataTempe[5]);
      } else if (yi <= dataTempe[7]) {
        FiberTemperature = dataTempe[4] - (dataTempe[5] - yi) * (dataTempe[4] - dataTempe[6]) /
                                              (dataTempe[5] - dataTempe[7]);
      } else if (yi <= dataTempe[9]) {
        FiberTemperature = dataTempe[6] - (dataTempe[7] - yi) * (dataTempe[6] - dataTempe[8]) /
                                              (dataTempe[7] - dataTempe[9]);
      } else if (yi <= dataTempe[11]) {
        FiberTemperature = dataTempe[8] - (dataTempe[9] - yi) * (dataTempe[8] - dataTempe[10]) /
                                              (dataTempe[9] - dataTempe[11]);
      } else if (yi <= dataTempe[13]) {
        FiberTemperature = dataTempe[10] - (dataTempe[11] - yi) * (dataTempe[10] - dataTempe[12]) /
                                               (dataTempe[11] - dataTempe[13]);
      } else if (yi <= dataTempe[15]) {
        FiberTemperature = dataTempe[12] - (dataTempe[13] - yi) * (dataTempe[12] - dataTempe[14]) /
                                               (dataTempe[13] - dataTempe[15]);
      } else if (yi <= dataTempe[17]) {
        FiberTemperature = dataTempe[14] - (dataTempe[15] - yi) * (dataTempe[14] - dataTempe[16]) /
  ...
  sTData[1] =
      fabs(dataTempe[0] - dataTempe[16]) * 12e-6 * fabs(ThermalTangent[0]) * h * h / 12 / 0.7;
  return *sT;
}
...
MembranePlateFiberSectionThermal::sendSelf(int commitTag, Channel& theChannel)
{
  ...
  if (res < 0) {
    opserr << "WARNING MembranePlateFiberSectionThermal::sendSelf() - " << this->getTag()
           << " failed to send ID\n";
...
int
MembranePlateFiberSectionThermal::recvSelf(int commitTag, Channel& theChannel,
                                           FEM_ObjectBroker& theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING MembranePlateFiberSectionThermal::recvSelf() - " << this->getTag()
  ...
  // Number of materials is the same, receive materials into current space
    ...
    for (i = 0; i < 5; i++) {
      ...
      res += theFibers[i]->recvSelf(commitTag, theChannel, theBroker);
      if (res < 0) {
        opserr << "MembranePlateFiberSectionThermal::recvSelf() - material " << i
...
Response*
MembranePlateFiberSectionThermal::setResponse(const char** argv, int argc, OPS_Stream& output)
{
