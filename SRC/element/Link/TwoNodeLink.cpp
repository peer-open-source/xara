...
TwoNodeLink::TwoNodeLink(int tag, int dim, 
                        int Nd1, int Nd2, 
                        const ID &direction, 
  ...
  , theMaterials(0), numDIR(direction.Size()), dir(0)
  , trans(T)
  , Mratio(Mr), shearDistI(sdI)
  , addRayleigh(addRay)
  , mass(mass), L(0.0)
  , onP0(true)
  , ub(0), ubdot(0), qb(0), ul(0)
  , Tgl(0,0), Tlb(0,0)
...
TwoNodeLink::TwoNodeLink()
  : Element(0, ELE_TAG_TwoNodeLink),
    ...
    theMaterials(0), numDIR(0), dir(0)
  , trans{}
  , Mratio(0), shearDistI(0)
  , addRayleigh(0), mass(0.0)
  , L(0.0)
...
int
TwoNodeLink::getNumExternalNodes() const
{
  return 2;
}
...
void
TwoNodeLink::setDomain(Domain *theDomain)
{
  ...
  static Matrix TwoNodeLinkM2( 2, 2);   // class wide matrix for 2*2
  static Matrix TwoNodeLinkM4( 4, 4);   // class wide matrix for 4*4
  static Matrix TwoNodeLinkM6( 6, 6);   // class wide matrix for 6*6
  static Matrix TwoNodeLinkM12(12,12);  // class wide matrix for 12*12
  static Vector TwoNodeLinkV2(2);   // class wide Vector for size 2
  static Vector TwoNodeLinkV4(4);   // class wide Vector for size 4
  static Vector TwoNodeLinkV6(6);   // class wide Vector for size 6
  static Vector TwoNodeLinkV12(12);  // class wide Vector for size 12
...
int
TwoNodeLink::revertToLastCommit()
{
  int errCode = 0;
...
const Matrix&
TwoNodeLink::getTangentStiff()
{
  static double kb_data[36], kl_data[144], kg_data[144];
  // zero the matrix
  theMatrix->Zero();

  // get resisting force and stiffness
  Matrix kb(kb_data,numDIR,numDIR);
  for (int i=0; i<numDIR; i++)  {
  ...
  Matrix kl(kl_data,numDOF,numDOF);
...
int
TwoNodeLink::addLoad(ElementalLoad *theLoad, double loadFactor)
{
...
int 
TwoNodeLink::addInertiaLoadToUnbalance(const Vector &accel)
{
...
const Vector&
TwoNodeLink::getResistingForceIncInertia()
{
...
int
TwoNodeLink::sendSelf(int commitTag, Channel &sChannel)
{
...
int
TwoNodeLink::recvSelf(int commitTag, Channel &rChannel, FEM_ObjectBroker &theBroker)
{
  ...
  if ((int)data(4) == 3)  {
    ...
    // TODO: set Transformation matrix from x
  }
  if ((int)data(5) == 3)  {
    Vector y;
    rChannel.recvVector(0, commitTag, y);
    // TODO: set Transformation matrix from y
  }
  if ((int)data(6) == 4)  {
      Mratio.resize(4);
...
void
TwoNodeLink::Print(OPS_Stream &s, int flag)
{
  if (flag == OPS_PRINT_CURRENTSTATE) {
    ...
    s << "  Mratio: " << Mratio << "  shearDistI: " << shearDistI << "\n";
    s << "  addRayleigh: " << addRayleigh << "  mass: " << mass << "\n";
...
Response* 
TwoNodeLink::setResponse(const char **argv, int argc, OPS_Stream &output)
{
...
int
TwoNodeLink::getResponse(int responseID, Information &eleInfo)
{
...
TwoNodeLink::setUp()
{
  ...
  if (xn == 0 || yn == 0 || zn == 0)  {
      opserr << "TwoNodeLink::setUp() - "
          << "element: " << this->getTag() << endln
          << "invalid orientation vectors, got "
...
// set transformation matrix from global to local system
void 
TwoNodeLink::setTranGlobalLocal()
{
...
void
TwoNodeLink::addPDeltaForces(Vector &pLocal, const Vector& qBasic)
{
  int dirID;
  double N = 0.0;
  double deltal1 = 0.0;
...
void 
TwoNodeLink::addPDeltaStiff(Matrix &kLocal, const Vector& qBasic)
{
  int dirID;
  double N = 0.0;
