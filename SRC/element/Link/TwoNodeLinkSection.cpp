    ...
    Tgl(0,0), Tlb(0,0), theMatrix(0), theVector(0), theLoad(0)
{
  ...
  if (numDIR < 1) {
    opserr << "TwoNodeLinkSection element: "
...
int
TwoNodeLinkSection::getNumExternalNodes() const
{
  return 2;
}
...
const ID&
TwoNodeLinkSection::getExternalNodes() 
{
...
Node**
TwoNodeLinkSection::getNodePtrs() 
{
  return theNodes;
}
...
void
TwoNodeLinkSection::setDomain(Domain *theDomain)
{
  ...
  static Matrix TwoNodeLinkSectionM4(4,4);
  static Matrix TwoNodeLinkSectionM6(6,6);
  static Matrix TwoNodeLinkSectionM12(12,12);
  static Vector TwoNodeLinkSectionV2(2);
  static Vector TwoNodeLinkSectionV4(4);
  static Vector TwoNodeLinkSectionV6(6);
  static Vector TwoNodeLinkSectionV12(12);
  ...
  if (dofNd1 != dofNd2)  {
      opserr << "nodes " << connectedExternalNodes(0) 
              << " and "  << connectedExternalNodes(1)
...
const Matrix&
TwoNodeLinkSection::getTangentStiff()
{
  // zero the matrix
  theMatrix->Zero();
...
const Vector&
TwoNodeLinkSection::getResistingForce()
{
  // zero the residual
  theVector->Zero();
...
const Vector& 
TwoNodeLinkSection::getResistingForceIncInertia()
{
...
int 
TwoNodeLinkSection::sendSelf(int commitTag, Channel &sChannel)
{
...
Response* 
TwoNodeLinkSection::setResponse(const char **argv, int argc, OPS_Stream &output)
{
...
int
TwoNodeLinkSection::getDirID(int sectionCode)
{
  int dirID = -1;
...
void
TwoNodeLinkSection::addPDeltaForces(Vector &pLocal, const Vector& qBasic)
{
  int dirID;
  double N = 0.0;
  double deltal1 = 0.0;
...
void 
TwoNodeLinkSection::addPDeltaStiff(Matrix &kLocal, const Vector& qBasic)
{
  int dirID;
  double N = 0.0;
...
int
TwoNodeLinkSection::setParameter(const char **argv, int argc, Parameter &param)
{
  int result = -1;
