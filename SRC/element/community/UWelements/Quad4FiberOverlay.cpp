...
void * OPS_ADD_RUNTIME_VPV(OPS_Quad4FiberOverlay)  
{
  ...
  if (theMaterial == 0) {
    opserr << "WARNING material with tag " << matTag << "not found for element " << eleTag << "\n";
    return 0;
  }

  // Parsing was successful, allocate the material
  return new Quad4FiberOverlay(iData[0], iData[1], iData[2], iData[3], iData[4], *theMaterial, dData[0], dData[1], dData[2]);
}
...
int
Quad4FiberOverlay::getNumExternalNodes() const
{
  return SL_NUM_NODE;
}

const ID &
Quad4FiberOverlay::getExternalNodes()
 {
   return externalNodes;
 }
 
Node **
Quad4FiberOverlay::getNodePtrs()
 {
   return theNodes;
 }
 
int
Quad4FiberOverlay::getNumDOF()
{
     return SL_NUM_DOF;
}
...
const Matrix &
Quad4FiberOverlay::getInitialStiff()
{
