...
Matrix LeadRubberY::theMatrix(12,12);
Vector LeadRubberY::theVector(12);


static int numMyBearing = 0;
static int tag = 0;  // Tag to identify if bearing has failed in buckling
void *OPS_ADD_RUNTIME_VPV(OPS_LeadRubberY)
{
    ...
    if (numArgs !=12 && numArgs !=18 && numArgs !=19 && numArgs !=20
        ...
        && numArgs !=31 && numArgs !=32 && numArgs !=33 && numArgs !=34) {
            opserr << "ERROR - LeadRubberY incorrect # args provided";
            return theEle;
    }
    ...
    if (ndm == 3) {
        ...
        if (ndf != 6)  {
            ...
            opserr << ", for space problem need 6 - LeadRubberY \n"; 
        }
        theEle = new LeadRubberY(iData[0], iData[1], iData[2], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6], dData[7], dData[8], y, x, kl, phi, al, sDratio, m, cd1, tc1, qL1, cL1, kS1, aS1, tag1, tag2, tag3, tag4, tag5);
    }
...
LeadRubberY::LeadRubberY(int eleTag, int Nd1, int Nd2, double qd, double alpha1, double Gr, double kbulk, double Di, double Do,
    ...
    Tgl(12,12), Tlb(6,12), ubC(6), zC(2), kbInit(6,6), theLoad(12)
{
    ...
    Fcr = -sqrt(1.5 * Pe * 1.5 * G * As);                   // Modified from G@100% to G@25%
...
int LeadRubberY::getNumExternalNodes() const
{
    return 2;
}


const ID& LeadRubberY::getExternalNodes()
{
    return connectedExternalNodes;
}


Node** LeadRubberY::getNodePtrs()
{
    return theNodes;
}


int LeadRubberY::getNumDOF()
{
    return 12;
}


void LeadRubberY::setDomain(Domain *theDomain)
{
    ...
    if (!theNodes[0] || !theNodes[1])  {
        if (!theNodes[0])  {
            opserr << "WARNING LeadRubberY::setDomain() - Nd1: "
                << connectedExternalNodes(0)
                << " does not exist in the model for";
        } else  {
            opserr << "WARNING LeadRubberY::setDomain() - Nd2: "
    ...
    if (dofNd1 != 6)  {
        opserr << "LeadRubberY::setDomain() - node 1: "
            << connectedExternalNodes(0)
            << " has incorrect number of DOF (not 6).\n";
        return;
    }
    if (dofNd2 != 6)  {
        opserr << "LeadRubberY::setDomain() - node 2: "
            << connectedExternalNodes(1)
            << " has incorrect number of DOF (not 6).\n";
        return;
    }
...
int LeadRubberY::commitState()
{
    ...
    if (tag2 == 1) {
        ...
        if (Ar / A <= 0.0 || uh / D2 > 1.0) {
            Fcrn = 0.2 * Fcr;
        }
        else {
            if (Ar / A > 0.0 && Ar / A < 0.6) {
                Fcrn = Fcr * (0.667 * Ar / A + 0.2);
            }
            else { Fcrn = Fcr * Ar / A; }
        }
    ...
    if (tag3 == 1) {
        ...
        //    opserr << "WARNING LeadRubberY::commitState() - Negative horizontal stiffness\n";
        //}
    }
...
int LeadRubberY::revertToLastCommit()
{
    return 0;
}


int LeadRubberY::revertToStart()
{
    int errCode = 0;
    ...
    if (sqrt(pow(delta_ub(1),2)+pow(delta_ub(2),2)) > 0.0)  {
        ...
        do  {
            ...
            if ((fabs(Df(0,0)) <= DBL_EPSILON) || (fabs(Df(1,1)) <= DBL_EPSILON))  {
                opserr << "WARNING: LeadRubberY::update() - "
...
const Matrix& LeadRubberY::getTangentStiff()
{
    // zero the matrix
    theMatrix.Zero();
...
const Matrix& LeadRubberY::getInitialStiff()
{
    // zero the matrix
    theMatrix.Zero();
...
const Matrix& LeadRubberY::getDamp()
{
    // zero the matrix
    theMatrix.Zero();
...
const Matrix& LeadRubberY::getMass()
{
    // zero the matrix
    theMatrix.Zero();
...
void LeadRubberY::zeroLoad()
{
    theLoad.Zero();
}


int LeadRubberY::addLoad(ElementalLoad *theLoad, double loadFactor)
{
    opserr <<"LeadRubberY::addLoad() - "
...
int LeadRubberY::addInertiaLoadToUnbalance(const Vector &accel)
{
    // check for quick return
    if (mass == 0.0)  {
        return 0;
    ...
    if (6 != Raccel1.Size() || 6 != Raccel2.Size())  {
        opserr << "LeadRubberY::addInertiaLoadToUnbalance() - "
...
const Vector& LeadRubberY::getResistingForce()
{
    // zero the residual
    theVector.Zero();
...
const Vector& LeadRubberY::getResistingForceIncInertia()
{
...
int LeadRubberY::sendSelf(int commitTag, Channel &sChannel)
{
...
int LeadRubberY::recvSelf(int commitTag, Channel &rChannel,
...
int LeadRubberY::displaySelf(Renderer &theViewer,
...
void LeadRubberY::Print(OPS_Stream &s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {
        // print everything
        s << "************************************************************" << "\n";
        s << "Element: " << this->getTag();
        s << "  type: LeadRubberY  iNode: " << connectedExternalNodes(0);
        ...
        s << " qL: " << qL << " cL: " << cL << " kS: " << kS << " aS: " << aS << "\n";
        s << "MECHANICAL PROPERTIES: HORIZONTAL MOTION" << "\n";
        s << "k0: " << k0 << " ke: " << ke << " qYield: " << qYield << " DeltaT: " << TL_commit << " Fcrmin: " << Fcrmin << "\n";
        s << "MECHANICAL PROPERTIES: VERTICAL MOTION" << "\n";
        s << "Kv: "<< Kv << " Fc: " << Fc << " Fcr: " << Fcr << " Fcn: " << Fcn << " umax: " << umax << "\n";
        // determine resisting forces in global system
        s << "  resisting force: " << this->getResistingForce() << "\n";
        s << "************************************************************" << "\n";
        //s <<"time: " << tCommit <<" ke0: " << G*A/Tr  <<" ke: " << ke <<" Fcr: "<< Fcr << " Fcrmin: "<< Fcrmin <<" Kv0: "<< Kv0<<" Kv: "<< Kv << " qYield0: " << qYield0 << " qYield: " << qYield << " DeltaT: " << TL_commit << "\n";
    }
...
Response* LeadRubberY::setResponse(const char **argv, int argc,
    OPS_Stream &output)
{
    Response *theResponse = 0;
    
    output.tag("ElementOutput");
    output.attr("eleType","LeadRubberY");
...
int LeadRubberY::getResponse(int responseID, Information &eleInfo)
{
...
void LeadRubberY::setUp()
{
    ...
    if (L > DBL_EPSILON)  {
        if (x.Size() == 0)  {
          ...
          opserr << "WARNING LeadRubberY::setUp() - "
...
double LeadRubberY::sgn(double x)
{
    if (x > 0)
        return 1.0;
    else if (x < 0)
...
double LeadRubberY::getCurrentTemp(double qYield, double TL_commit, double v)
{
    // lead core heating
    tCurrent = (this->getDomain())->getCurrentTime();
    if (tCurrent < tCommit) {
