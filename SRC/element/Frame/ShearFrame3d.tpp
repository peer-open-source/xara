...
template <std::size_t nen, int nwm>
ShearFrame3d<nen,nwm>::~ShearFrame3d()
{
...
template<std::size_t nen, int nwm>
int
ShearFrame3d<nen,nwm>::revertToStart()
{
...
template<std::size_t nen, int nwm>
int
ShearFrame3d<nen,nwm>::revertToLastCommit()
{
  pres = past;
...
ShearFrame3d<nen,nwm>::update()
{
  ...
  for (int i=0; i<nip; i++) {
    ...
    if (Linear) {
      R = pres[i].rotation;
      // R = dR + (pres[i].rotation - Eye3);
      pres[i].curvature += dtheta;
    }
    else {
      Matrix3D dR = ExpSO3(theta);
      R = dR*pres[i].rotation;
  ...
  //  pres[i].curvature = omega + TanSO3(theta, 'R')*dtheta;
    ...
    for (unsigned j=0; j<nen; j++) {
      MatrixND<nsr,ndf> Bj{};
      B_nat<nen,nwm>(Bj,  pres[i].shape, R*D, j);
      B[j] = A^Bj;
...
template<std::size_t nen, int nwm>
const Vector &
ShearFrame3d<nen,nwm>::getResistingForce()
{
...
template<std::size_t nen, int nwm>
const Matrix &
ShearFrame3d<nen,nwm>::getTangentStiff()
{
...
template<std::size_t nen, int nwm>
const Matrix &
ShearFrame3d<nen,nwm>::getInitialStiff()
{
...
template<std::size_t nen, int nwm>
const Vector &
ShearFrame3d<nen,nwm>::getResistingForceSensitivity(int grad)
{
  static constexpr Vector3D D {1, 0, 0};
...
template<std::size_t nen, int nwm>
int
ShearFrame3d<nen,nwm>::addLoad(ElementalLoad* theLoad, double loadFactor)
{
...
template<std::size_t nen, int nwm>
Response*
ShearFrame3d<nen,nwm>::setResponse(const char** argv, int argc, OPS_Stream& output)
{
...
template<std::size_t nen, int nwm>
int
ShearFrame3d<nen,nwm>::getResponse(int responseID, Information &info)
{
...
template<std::size_t nen, int nwm>
void
ShearFrame3d<nen,nwm>::Print(OPS_Stream& stream, int flag)
{
  const ID& node_tags = this->getExternalNodes();
...
template<std::size_t nen, int nwm>
int
ShearFrame3d<nen,nwm>::setParameter(const char** argv, int argc, Parameter& param)
{
  if (argc < 1)
    return -1;
...
template<std::size_t nen, int nwm>
int
ShearFrame3d<nen,nwm>::updateParameter(int parameterID, Information& info)
{
  return -1;
}
...
template<std::size_t nen, int nwm>
int
ShearFrame3d<nen,nwm>::activateParameter(int passedParameterID)
{
