...
//===----------------------------------------------------------------------===//
//
// CosseratFrame3d02 implementation.
// See CosseratFrame3d02.h for a summary.
//
//===----------------------------------------------------------------------===//
#include <cstddef>
#include "CosseratFrame3d02.h"
#include <Node.h>
#include <Matrix.h>
#include <Vector.h>
#include <MatrixND.h>
...
#include "Exact/ExactInterpolation.h"
...
CosseratFrame3d02<nen, nwm>::CosseratFrame3d02(int tag,
  ...
  : FiniteElement<nen, ndm, ndf>(tag, 0, nodes, 1),
    ...
    reference_rotation{},
    reference_curvature{},
    transform(tb.template create<nen, ndf>(0, param)),
...
template <std::size_t nen, int nwm>
CosseratFrame3d02<nen,nwm>::~CosseratFrame3d02()
{
...
template<std::size_t nen, int nwm>
int
CosseratFrame3d02<nen,nwm>::setNodes()
{
...
template<std::size_t nen, int nwm>
int
CosseratFrame3d02<nen,nwm>::revertToStart()
{
...
template<std::size_t nen, int nwm>
int
CosseratFrame3d02<nen,nwm>::revertToLastCommit()
{
  pres = past;
...
template<std::size_t nen, int nwm>
int
CosseratFrame3d02<nen,nwm>::update()
{
  ...
  for (unsigned i=0; i < nen; i++) {
    Vector3D dtheta_i = transform->getNodeRotationLogarithm(i);
...
template<std::size_t nen, int nwm>
const Vector &
CosseratFrame3d02<nen,nwm>::getResistingForce()
{
...
template<std::size_t nen, int nwm>
const Matrix &
CosseratFrame3d02<nen,nwm>::getTangentStiff()
{
...
template<std::size_t nen, int nwm>
const Matrix &
CosseratFrame3d02<nen,nwm>::getInitialStiff()
{
...
template<std::size_t nen, int nwm>
const Matrix &
CosseratFrame3d02<nen,nwm>::getMass()
{
...
template<std::size_t nen, int nwm>
const Vector &
CosseratFrame3d02<nen,nwm>::getResistingForceSensitivity(int grad)
{
...
template<std::size_t nen, int nwm>
int
CosseratFrame3d02<nen,nwm>::addLoad(ElementalLoad* theLoad, double loadFactor)
{
...
template<std::size_t nen, int nwm>
Response*
CosseratFrame3d02<nen,nwm>::setResponse(const char** argv, int argc, OPS_Stream& output)
{
...
template<std::size_t nen, int nwm>
int
CosseratFrame3d02<nen,nwm>::getResponse(int responseID, Information &info)
{
  double L = 0.0;
  if (this->setState(State::Init) == 0) {
    auto& theNodes = this->FiniteElement<nen,3,6+nwm>::theNodes;
...
template<std::size_t nen, int nwm>
void
CosseratFrame3d02<nen,nwm>::Print(OPS_Stream& stream, int flag)
{
  const ID& node_tags = this->getExternalNodes();
...
template<std::size_t nen, int nwm>
int
CosseratFrame3d02<nen,nwm>::setParameter(const char** argv, int argc, Parameter& param)
{
  if (argc < 1)
    return -1;
...
template<std::size_t nen, int nwm>
int
CosseratFrame3d02<nen,nwm>::updateParameter(int parameterID, Information& info)
{
  return -1;
}
...
template<std::size_t nen, int nwm>
int
CosseratFrame3d02<nen,nwm>::activateParameter(int passedParameterID)
{
