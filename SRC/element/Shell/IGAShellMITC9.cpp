...
// set domain
void
IGAShellMITC9::setDomain( Domain *theDomain ) 
{
  int i,j ;
  static Vector eig(3) ;
...
void IGAQuad::GaussRule(int NGPs1, Vector* Xg, Vector* WXg)//Verified
{
  ...
  for (int j = 1; j <= iter;j++) {
      ...
      for (int k = 2; k <= NGPs1; k++) {
          Vector pkp1 = 2 * IGA::Dot(x0, pk) - pkm1 - (IGA::Dot(x0, pk) - pkm1) / (double)k;
          pkm1 = pk;
          pk = pkp1;
      }
      //Vector Test = IGA::Dot(x0, x0);
      Vector den = -1 * IGA::Dot(x0, x0) + 1;
      d1 = NGPs * (pkm1 - IGA::Dot(x0, pk));
      dpn  = IGA::DotDiv(d1, den);
      d2pn = IGA::DotDiv((2 * IGA::Dot(x0, dpn) - e1 * pk), den);
      d3pn = IGA::DotDiv((4 * IGA::Dot(x0, d2pn) + (2 - e1) * dpn), den);
      d4pn = IGA::DotDiv((6 * IGA::Dot(x0, d3pn) + (6 - e1) * d2pn), den);
      Vector uu = IGA::DotDiv(pk, dpn);
      Vector vv = IGA::DotDiv(d2pn, dpn);
      //Initial approximation H
      h = IGA::Dot(-1 * uu, 0.5 * IGA::Dot(uu, vv + IGA::Dot(uu, IGA::Dot(vv, vv) - IGA::DotDiv(IGA::Dot(uu, d3pn), 3 * dpn))) + 1);
      //Refine H using one step of Newton's method
      Vector p = pk + IGA::Dot(h, dpn + 0.5 * IGA::Dot(h, d2pn + IGA::Dot(h / 3, d3pn + 0.25 * IGA::Dot(h, d4pn))));
      Vector dp = dpn + IGA::Dot(h, d2pn + 0.5 * IGA::Dot(h, d3pn + IGA::Dot(h, d4pn / 3)));
      h -= IGA::DotDiv(p, dp);
      x0 += h;
  }
  ...
  Vector fx = d1 - IGA::Dot(h, e1 * (pk + 0.5 * IGA::Dot(h, dpn + IGA::Dot(h / 3, d2pn + 0.25 * IGA::Dot(h, d3pn + 0.2 * IGA::Dot(h, d4pn))))));
  for (int i = 1;i <= x0.Size();i++) {
...
void
IGAQuad::DerBasisFuns(int Idx, Vector Pts, int obf, int n, Vector KnotVect, Matrix** N0n)
{
...
void 
IGAQuad::Rationalize(Vector WeightsCP, Vector N0, Matrix N1, Vector* R0, Matrix* R1)
{
...
int 
IGAQuad::findSpan(int order, int ncp, int eleId, int nele, Vector KnotVect) {
  ID Idxs(nele);
  Idxs.Zero();
  int iE = 1;
