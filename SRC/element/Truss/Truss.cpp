...
Truss::Truss(int tag, int dim, int Nd1, int Nd2, UniaxialMaterial& theMat, double a, double r,
...
Truss::getTangentStiff()
{
  ...
  for (int i = 0; i < dimension; i++) {
    for (int j = 0; j < dimension; j++) {
      double temp                     = cosX[i] * cosX[j] * EAoverL;
      stiff(i, j)                     = temp;
...
Truss::getResponse(int responseID, Information& eleInfo)
{
  ...
  case 2: fVec(0) = A * theMaterial->getStress(); return eleInfo.setVector(fVec);
  ...
  case 2000: return eleInfo.setDouble(A * L * theMaterial->getEnergy());
...
{
  switch (parameterID) {
  case 1:  A = info.theDouble; return 0;
...
Truss::getResistingForceSensitivity(int gradNumber)
{
  ...
  if (parameterID == 1) { // Cross-sectional area
    ...
    // Density, material parameter or nodal coordinate
    for (int i = 0; i < dimension; i++) {
...
Truss::commitSensitivity(int gradNumber, int numGrads)
{
  ...
  if (nodeParameterID0 != 0 || nodeParameterID1 != 0) {
    ...
    if (nodeParameterID0 == 1) { // here x1 is random
      double temp1 = (-L + dx * dx / L) / (L * L);
      double temp2 = dx * dy / (L * L * L);
      //dtdh(0) = -temp1;
      //dtdh(1) = -temp2;
      //dtdh(2) = temp1;
      //dtdh(3) = temp2;
