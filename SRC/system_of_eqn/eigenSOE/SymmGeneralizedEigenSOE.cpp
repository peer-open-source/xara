...
int SymmGeneralizedEigenSOE::getNumEqn() const
{
    return size;
}
...
int
SymmGeneralizedEigenSOE::addM(const Matrix &m, const ID &id, double fact)
{
    // check for quick return 
    if (fact == 0.0)
        return 0;
    ...
    if (fact == 1.0) { // do not need to multiply 
        for (int i=0; i<idSize; i++) {
            ...
            if (col < size && col >= 0) {
                ...
                for (int j=0; j<idSize; j++) {
                    int row = id(j);
                    if (row <size && row >= 0 && row <= col) { // Only add upper
...
void SymmGeneralizedEigenSOE::zeroA()
{
    double *Aptr = A;
    for (int i = 0; i < Asize; i++)
        *Aptr++ = 0;
...
void SymmGeneralizedEigenSOE::zeroM()
{
    double *Mptr = M;
    for (int i = 0; i < Msize; i++)
        *Mptr++ = 0;
...
int
SymmGeneralizedEigenSOE::sendSelf(int commitTag, Channel &theChannel)
{
    return 0;
}
...
int
SymmGeneralizedEigenSOE::recvSelf(int commitTag, Channel &theChannel, 
